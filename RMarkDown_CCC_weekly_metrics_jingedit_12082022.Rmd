...
date: "`r format(Sys.Date()-9, '%d %B, %Y')`"
...
This is a CCC weekly report with tables and plots of Connect recruitments: recruitment process, incentive, demographics, survey modules, biospecimen collections, refusal and withdrawals on `r format(Sys.Date()-92, '%d %B, %Y')` among the participants without inactive recruits.
...
recr_noinact_wl1 <- readRDS("~/Documents/CONNECT_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/recruit2023-01-23.rds")
dictionary <- rio::import("https://episphere.github.io/conceptGithubActions/aggregate.json",format = "json") ###mastr DD for all the CIDs in
dd <-rbindlist(dictionary,fill=TRUE,use.names=TRUE,idcol="CID") #4379 including duplicates for CID
dd$`Variable Label` <- ifelse(is.na(dd$`Variable Label`), dd$`Variable Name`, dd$`Variable Label`)
#dd <- as.data.frame(do.call("rbind",dictionary))#3523, some CID and labels are linked to different variable names 
...
# saveRDS(recr_noinact_wl1,paste("~/Documents/CONNECT_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/recruit",currentDate,".rds\")", sep=""))
  ...
  #saveRDS(recr_noinact_wl1,"~/Documents/CONNECT_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/recruit01092023.rds")
...
recruittypetotal <- as.data.frame(tab1(data[which(!is.na(data$site)),]$reruit_type)[[2]])
...
colnames(reasons.3a)[1] <- "Site"
colnames(reasons.3a)[10:11] <- c("Cannot Be Verified","Total Active UP Submitted")
colnames(reasons.3a)[2:9] <- gsub("n_pct_356674370|n_pct_539025306","",colnames(reasons.3a)[2:9])
...
colnames(reasons.3p)[10:11] <- c("Cannot Be Verified","Total Passive UP Submitted")
colnames(reasons.3p)[2:9] <- gsub("n_pct_356674370|n_pct_539025306","",colnames(reasons.3p)[2:9])
colnames(reasons.3p)[1] <- "Site"
...
optouts1_all[is.na(optouts1_all)] = "0 (0%)"
...
grab <- dd %>% filter(grepl(paste(optouts1_transpose$CID,collapse = "|"),dd$CID)) %>% select(CID,`Variable Label`)
...
Table7_optreasons[26,2] <- "Total Opt-Outs"
...
for(site in unique(data[which(data$d_827220437>0),]$site)){
  ...
  Total <- nrow(subs) ##active recruits per site
  campaign <- pct_tb(subs$active_camptype,subs$Rcrt_Verified_v1r0) #to calculate the percentage of verified with each mode (yes)
  campaign$N_active <- format(campaign$n_104430631 + campaign$n_353358909,big.mark = ",")
  colnames(campaign) <- paste(site,colnames(campaign),sep="_")
  campaign$campaign_type <- rownames(campaign)
  campaign_site[[site]] <- as.data.frame(campaign[,which(grepl("n_pct_353358909|N_active|n_353358909",colnames(campaign)) | colnames(campaign) =="campaign_type")])
  #campaign.site <- cbind(campaign.site,campaign[,which(grepl("n_pct_353358909|N_active",colnames(campaign)) | colnames(campaign) =="campaign_type")])
} 

campaign.site <- campaign_site %>% purrr::reduce(full_join, by='campaign_type') 
campaign.site$Total.Verified <- format(rowSums(campaign.site[,grep("n_353358909",colnames(campaign.site))],na.rm=TRUE),big.mark = ",")
campaign.site1 <- campaign.site[order(campaign.site$campaign_type),] %>% select(campaign_type,Total.Verified,contains("n_pct_353358909"))

##table with percentages
campaign.site1[is.na(campaign.site1)] = "0 (0 %)"
colnames(campaign.site1)[c(3:11)] <- gsub("_n_pct_353358909"," ( %)",colnames(campaign.site1)[c(3:11)])
campaign.site1.t <- as.data.frame(t(campaign.site1[,c(3:11,2)]))
names(campaign.site1.t) <- campaign.site1$campaign_type

##table as Frequencies # keep in the weekly report ##to remove the percentage from the whole table;

campaign.site.n <- campaign.site[order(campaign.site$campaign_type),] %>% select(campaign_type,Total.Verified,contains("n_353358909")) 
campaign.site.n[is.na(campaign.site.n)] = 0

colnames(campaign.site.n)[c(3:11)] <- gsub("_n_353358909","",colnames(campaign.site.n)[c(3:11)])
campaign_site.t <- as.data.frame(t(campaign.site.n[,c(3:11,2)]))
colnames(campaign_site.t) <- campaign.site.n$campaign_type
...
table9_Totalactivecamp <-  data %>% filter(d_512820379 == 486306141 & d_827220437 >0) %>%
  select(site, active_camptype) %>% tbl_cross( row=site,
    col = active_camptype,
    label=active_camptype~"Campaign Type of Active Recruits",
    percent = "row",
    digits=c(0,2),
    missing = "no",
...
table9_Totalactivecamp <- table9_Totalactivecamp %>%
  ...
  #italicize_levels() %>% 
  modify_header( ) %>%
  modify_caption("Table 10. Total Number of Active Recruits in Campaign") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
campaign2 <- list()
campaign2_site  <- list() 
data$Rcrt_Verified_v1r0 <- factor(data$Rcrt_Verified_v1r0,levels=c(104430631,353358909))
#campaign.site <- NULL
...
for(site in unique(data[which(data$d_827220437>0),]$site)){
  ...
  subs <- data[which(data$site == site & data$d_512820379 == 486306141 & data$d_827220437>0 ),]
  Total <- nrow(subs) ##active recruits per site
  cross.tb <- CrossTable(subs$active_camptype,subs$Rcrt_Verified_v1r0)
  cross_tb.f <- as.data.frame(cbind(cross.tb$t,cross.tb$prop.row))
  #to calculate the percentage of verified with each mode (yes)
  if(ncol(cross_tb.f)==2 & colnames(cross_tb.f)[1]=="104430631"){
    cross_tb.f$N_active <- cross_tb.f[,1]
    cross_tb.f$N_active_pct <- paste0(cross_tb.f[,1], "( 0.00 %)",sep=" ")
  }
 if(ncol(cross_tb.f)==2 & colnames(cross_tb.f)[1]=="353358909"){
    cross_tb.f$N_active <- cross_tb.f[,1]
    cross_tb.f$N_active_pct <- paste0(cross_tb.f[,1], "( 100.00 %)",sep=" ")}
  if(ncol(cross_tb.f)==4){
    cross_tb.f$N_active <- cross_tb.f[,1]+cross_tb.f[,2]
    cross_tb.f$N_active_pct <- paste0(format(cross_tb.f[,1]+ cross_tb.f[,2],big.mark = ","), " (",round(100*cross_tb.f[,4],2), " %)",sep=" ")}
    
  colnames(cross_tb.f) <- paste(site,colnames(cross_tb.f),sep="_")
  cross_tb.f$campaign_type <- rownames(cross_tb.f)
  n <- as.numeric(ncol(cross_tb.f))
  campaign2[[site]] <- cross_tb.f
  
  cross_tb.fn <- cross_tb.f[,c(n-2,n-1,n)]
  #total <- paste0(format(sum(cross_tb.f[,1])+ sum(cross_tb.f[,2]),big.mark = ","), " (", round(100*sum(cross_tb.f[,2])/(sum(cross_tb.f[,1]) + sum(cross_tb.f[,2])),2), " %)", sep=" ")
  total <- format(sum(cross_tb.f[,1])+ sum(cross_tb.f[,2]),big.mark = ",")
  
  campaign2_site[[site]] <- rbind(cross_tb.f[,c(n-1,n)],c(total,"Total"))
  
} 

campaign.veri <- campaign2_site %>% reduce(full_join,by="campaign_type")
campaign.veri[is.na(campaign.veri)] = "0 (0.00 %)"
colnames(campaign.veri)[c(1,3:10)] <- gsub("_N_active_pct","",colnames(campaign.veri)[c(1,3:10)])

total.veri_pct <- data[which(data$d_512820379==486306141),] %>%  group_by(active_camptype,Rcrt_Verified_v1r0) %>% tally()
act.veri.camp <- dcast(total.veri_pct,active_camptype ~ Rcrt_Verified_v1r0, value.var = "n")
 act.veri.camp[is.na(act.veri.camp)] <- 0
 act.veri.camp$n_pct_active <- paste0(format(act.veri.camp$`353358909` + act.veri.camp$`104430631`,big.mark=","), " ( ", round(100*act.veri.camp$`353358909`/(act.veri.camp$`104430631`+ act.veri.camp$`353358909`),2), " %)",sep="")   
act.veri.camp <- act.veri.camp[order(act.veri.camp$active_camptype),] 


campaign.veri <- campaign.veri[order(campaign.veri$campaign_type),]
campaign.veri.all <- merge(campaign.veri,act.veri.camp,by.x="campaign_type",by.y="active_camptype",all.x=TRUE)
campaign.veri.all$n_pct_active[is.na(campaign.veri.all$n_pct_active)] =  format(sum(total.veri_pct$n),big.mark=",")              

campaign.veri.all <- campaign.veri.all %>% dplyr::rename( Total.Active.Recruits=n_pct_active)    
campaign_veri.t <- as.data.frame(t(campaign.veri.all[,c(2:10,13)]))
colnames(campaign_veri.t) <- campaign.veri$campaign_type
colnames(campaign_veri.t)[9] <- "Total Active Recruits"



    #as.data.frame(cross_tb.f[,which(grepl("n_pct_353358909|N_active|n_353358909",colnames(cross_tb.f)) |        colnames(campaign) =="campaign_type")])
  #campaign.site <- cbind(campaign.site,campaign[,which(grepl("n_pct_353358909|N_active",colnames(campaign)) | colnames(campaign) =="campaign_type")])
...
recruit_timing$tm_signin_cst <- difftime(recruit_timing$consent_time,recruit_timing$signin_time,units="days")
recruit_timing$tm_consent_prof <- difftime(recruit_timing$profile_time,recruit_timing$consent_time,units="days")
...
table11_Tm_signin_cst <- recruit_timing %>% filter( d_827220437 >0 & 
  ...
  group_by(d_512820379 == 486306141) %>% 
...
table11_Tm_signin_cst$recruitment_type <- ifelse(table11_Tm_signin_cst$`d_512820379 == 486306141`=="TRUE", "Active", "Passive")
...
table11_Tm_signin_cst.act1 <- table11_Tm_signin_cst.act1[order(table11_Tm_signin_cst.act1$order),c(1,13,4:11)]
...
table11_Tm_signin_cst.pas <- recruit_timing %>% 
  filter(d_512820379 == 854703046 & d_827220437 >0 & d_230663853==353358909 & d_919254129== 353358909 & !is.na(tm_signin_cst)
         ) %>% 
  mutate(time = as.numeric(tm_signin_cst),
         site = droplevels(site)) %>% # Summary by group using dplyr, to make the time units as hours
  group_by(site) %>% 
...
table11_Tm_signin_cst.pas <- table11_Tm_signin_cst.pas[order(as.numeric(table11_Tm_signin_cst.pas$order)),c(1,13,3,5:11)]
colnames(table11_Tm_signin_cst.pas)[1:3] <- c("Site","Recruitment Type","Number Consent")
...
colnames(table12_Tm_consent_prof.act1)[4] <- "Number UP submitted"
...
colnames(table12_Tm_consent_prof.pas)[4] <- "Number UP submitted"
...
prof_verif <- recruit_timing %>% filter(d_827220437 >0 & d_230663853==353358909 & d_919254129== 353358909 & d_699625233 == 353358909 & d_821247024 != 875007964 ) %>% group_by(d_512820379,site) %>% tally()
...
colnames(table13_Tm_prof_verif.pas)[4] <- "Number Verification"
...
table14_Tm_enroll_verif.act <- rbind(table14_Tm_enroll_verif.act,table14_Tm_enroll_verif[which(table14_Tm_enroll_verif$recruitment_type=="Active"),c(2:10)]) 
...
table14_Tm_enroll_verif.pas$`Number Verification Complete` <- ifelse(table14_Tm_enroll_verif.pas$site =="Total", sum(table14_signin_verif.pas$`Number Verification Complete`),table14_Tm_enroll_verif.pas$`Number Verification Complete` )
...
signin_methods1 <- signin_methods1 %>%
  ...
  modify_caption("Table 17. Sign-In Mechanism for First Log on") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE) 
...
Table16_awaresreasons$Awareness <- ifelse(is.na(Table16_awaresreasons$CID),"Total",ifelse(Table16_awaresreasons$CID == 0, "No Response Sent",Table16_awaresreasons$`Variable Label`))
...
Table16b_awaresreasons$Awareness <- ifelse(is.na(Table16b_awaresreasons$CID),"Total",ifelse(Table16b_awaresreasons$CID == 0, "No Response Sent",Table16b_awaresreasons$`Variable Label`))
...
chiago_ince <- incentive %>% filter(d_827220437 == 809703864 & d_821247024==197316935) %>% 
  ...
  chica_ince <- chiago_ince %>% filter(d_821247024==197316935) %>% 
                                    select(Incentive_Eligible,Payment_Issued,Payment_Refused) %>% 
  ...
  chica_ince[["table_body"]]$n <- ifelse(is.na(chica_ince[["table_body"]]$stat_1), NA, ifelse(chica_ince[["table_body"]]$stat_1=="0  (0.00%)", 0, 464))
    
  
  chica_ince <- chica_ince  %>% 
    modify_header(label="") %>%
    modify_caption("Table 19. Distribution Of Baseline Incentives Among Verified Participants from Chicago") %>%
    as_kable_extra(escape = FALSE, addtl_fmt = TRUE)

##issued and refused
chia_issue_refused <- chiago_ince %>% 
  dplyr::filter(d_821247024==197316935 & Incentive_Eligible == "Incentive Eligible") %>%                                     select(Payment_Issued,Payment_Refused) %>% 
  tbl_cross(col=Payment_Issued,
            row =Payment_Refused,
            digits=c(0,2),
            percent = "row",
            label=list(Payment_Issued ~"Eligible Incentive Payment Issued",Payment_Refused ~"Payment Refused"),
            missing="ifany",
            missing_text="0 (0.00%)",
            margin_text = "Total Verified Participants") 
  
chia_issue_refused[["table_body"]]$stat_0 <- sapply(strsplit(chia_issue_refused[["table_body"]]$stat_0," "),"[",1)
chia_issue_refused[["table_body"]] <- chia_issue_refused[["table_body"]] %>% mutate(stat_0= gsub("NA%","0.00%",stat_0),
                                                                                    stat_1= gsub("NA%","0.00%",stat_1),
                                                                                    stat_2 = gsub("NA%","0.00%",stat_2)) 

chia_issue_refused <- chia_issue_refused %>%
  modify_header(label ~"Eligible Incentive") %>%
  modify_caption("Table 19a. Baseline Eligible Incentives Payments Issued vs Refused Among Verified Participants from Chicago") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
  #bold_labels() %>%
  #italicize_levels() %>% 
...
nonchico_inc[["table_body"]]$n <- ifelse(is.na(nonchico_inc[["table_body"]]$stat_1), NA, ifelse(nonchico_inc[["table_body"]]$stat_1!="0  (0.00%)" | nonchico_inc[["table_body"]]$stat_2 !="0  (0.00%)", format(as.numeric(na2zero(sapply(strsplit(nonchico_inc[["table_body"]]$stat_1," "),"[[",1)))+ as.numeric(na2zero(sapply(strsplit(gsub(",","",nonchico_inc[["table_body"]]$stat_2)," "),"[[",1))), big.mark=",")))
...
#   nonchia_issue_refused <- incentive %>% 
#     dplyr::filter(d_827220437 != 809703864 & d_827220437 >0 & d_821247024==197316935) %>% 
#   select(Payment_Issued,Payment_Refused) %>% 
#   tbl_cross(col=Payment_Issued,
#             row =Payment_Refused,
#             digits=c(0,2),
#             percent = "row",
#             label=list(Payment_Issued ~"Payment Issued",Payment_Refused ~"Payment Refused"),
#             missing="ifany",
#             missing_text="0 (0.00%)",
#             margin_text = "Total Verified Participants") 
#   
# nonchia_issue_refused[["table_body"]]$stat_0 <- sapply(strsplit(nonchia_issue_refused[["table_body"]]$stat_0," "),"[",1)
# # nonchia_issue_refused[["table_body"]] <- chia_issue_refused[["table_body"]] %>% mutate(stat_0= gsub("NA%","0.00%",stat_0),
# #                                                                                     stat_1= gsub("NA%","0.00%",stat_1),
# #                                                                                     stat_2 = gsub("NA%","0.00%",stat_2)) 
# 
# nonchia_issue_refused <- nonchia_issue_refused %>%
#   modify_header(label ~"Eligible Incentive") %>%
#   modify_caption("Table 19a. Baseline Eligible Incentives Payments Issued vs Refused Among Verified Participants from NORC=Issues Incentives") %>%
#   as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
#   #bold_labels() %>%
#   #italicize_levels() %>% 
...
nonch_Refused_issued <- incentive %>% filter(d_827220437 != 809703864 & d_827220437 >0 & d_821247024==197316935
                                       & Incentive_Eligible == "Incentive Eligible"  ) %>%
  select(Payment_Refused,Payment_Issued) %>%
  tbl_cross(col = Payment_Issued,
            row = Payment_Refused,
            label =list(Payment_Issued ~"Eligible Incentive Payment",
                        Payment_Refused ~"Eligible Incentive Payment"),
            digits=c(0,2),
            percent = "col",
...
nonch_Refused_issued[["table_body"]]$stat_0 <- sapply(strsplit(nonch_Refused_issued[["table_body"]]$stat_0," "),"[",1)

nonch_Refused_issued <- nonch_Refused_issued %>%
  #modify_header() %>%
  modify_caption("Table 20a. Baseline Eligible Incentives vs Issued Payments Among Verified Participants from NORC") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
  #bold_labels() %>%
  #italicize_levels() %>% 

# nonch_eligible_refused <- nonch_eligible_issued %>% dplyr::filter(d_821247024==197316935) %>% 
...
#             missing_text = "(Missing)")  
# 
# nonch_eligible_refused[["table_body"]]$stat_0 <- sapply(strsplit(nonch_eligible_refused[["table_body"]]$stat_0," "),"[",1)
# 
# nonch_eligible_refused <- nonch_eligible_refused %>%
#   modify_header(label ~"Incentive Status") %>%
#   modify_caption("Table 21b. Baseine Eligible Incentives vs Payment Refusals Among Verified Participants from NORC") %>%
#   as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
# 
#   
...
tb_refwd <- tb_refwd %>% 
  modify_header() %>%
  modify_caption("Table 22. Refusals and Withdrawals Among All Consented Participants") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
refwd_reasons.all <- as.data.frame(t(colSums(ref_wd_reasons1[,c(2:11)], na.rm=TRUE )))
...
Modbiospe.All <- data %>% filter(d_821247024==197316935 & d_827220437>0) %>% 
  ...
  tbl_cross(col = baseMcomplt_all,
            row = biospeDonation,
            label = list(baseMcomplt_all~"Completion of Baseline Surveys Completion",
...
Modbiospe.All <- Modbiospe.All %>%
  ...
  #italicize_levels() %>% 
  modify_header() %>%
  modify_caption("Table 24. Completion of Baseline Activities among Verified Participants for All Sites") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE) 
...
##opt-out: d_158291096
tb.list <- c("Rcrt_Verified_v1r0","d_130371375_d_266600170_d_731498909","state_d_158291096","invitations","withdrawal")
...
colnames(Table_cmplt)[2:6] <- c("Verified Participants","Complete Participants", "Invitations", "Opt Outs", "withdrawals")
```
...
#including Tables
knitr::kable(recruittypetotal[,c(4,1:3)],format.args = list(big.mark = ","),booktabs = T, caption = 'Table 1. All Records in Connect', row.names=FALSE, align=c("l", "c", "c", "c")) %>% footnote(general='Not Active records are excluded from this report')
...
colnames(reasons1a)[1] <- "Site"
knitr::kable(reasons1a[,c(1:9,11,10)], caption = 'Table 5a. Reason Cannot Be Verified among Total UP Submitted', row.names=FALSE, align=c("l", "c", "c","c", "c", "c", "c", "c", "c", "c"))%>% kable_styling(latex_options = "scale_down")

knitr::kable(reasons.3a, caption = 'Table 5b. Reason Cannot Be Verified among Active UP Submitted', row.names=FALSE, align=c("l", "c", "c","c", "c", "c", "c", "c", "c", "c","c"))%>% kable_styling(latex_options = "scale_down")

knitr::kable(reasons.3p, caption = 'Table 5c. Reason Cannot Be Verified among Passive UP Submitted', row.names=FALSE, align=c("l", "c", "c","c", "c", "c", "c", "c", "c", "c","c"))%>% kable_styling(latex_options = "scale_down")

knitr::kable(reasons.notveri, caption = 'Table 5d. Reason Cannot Be Verified among Not Verified Recruits', row.names=FALSE, align=c("l", "c", "c","c", "c", "c", "c", "c", "c", "c"))%>% kable_styling(latex_options = "scale_down") %>% footnote(general="Age match or Site match are not available among this 'Cannot Be Verified' group")

knitr::kable(cancer.veri.up[,c(1,8,5,2,3,7)],caption = 'Table 6. Previous Cancer Reported in User Profile by Verification Status', col.names=c("Site","Total Previous Cancer","Verified","Cannot be verified","Duplicates","Total Recruits that Submitted UP"), row.names=FALSE,format.args = list(big.mark = ","), align=c("l","c","c","c","c","c") ) %>%  add_header_above(header = c(" " = 2, "Previous Cancer by Verification Status" = 3, " "=1)) %>% landscape()

knitr::kable(optout1.sites[,c(9,4,8,7)],caption = 'Table 7. Total Number of Opt-outs by Site',col.names=c("Site","Total Opt-Outs", "Percent %", "Total Active Recruits"), format.args = list(big.mark = ","), row.names=FALSE, align=c("l","c","c","c"))

knitr::kable(Table7_optreasons[,c(2:12),],caption = 'Table 8. Pre-Consent Opt-Out Reasons Among Those that Opted Out', row.names=FALSE, align=c("l", "c", "c","c", "c", "c", "c", "c", "c", "c","c","c"))%>%  kable_styling(latex_options = "scale_down")%>% landscape() %>% footnote(general='UChicogo and KP sites are not currently sending data on opt-outs')

colnames(campaign_site.t)[9] <- "Total Active Verified"

knitr::kable(campaign_site.t,caption = 'Table 9. Number of Active Verified Recruits by Campaign Types',row.names=TRUE, booktabs = T,align=c("l", "c", "c","c", "c", "c", "c", "c", "c","c")) %>% kable_styling(latex_options = "scale_down") %>%landscape() 

table9_Totalactivecamp %>% kable_styling(latex_options = "scale_down")%>% landscape()

knitr::kable(campaign_veri.t,caption = 'Table 11. Total Number of Active Recruits with Percentage of Success on Campaign Types of Verified Participants',row.names=TRUE, booktabs = T,align=c("l", "c", "c","c", "c", "c", "c", "c", "c")) %>% kable_styling(latex_options = "scale_down") %>%landscape() %>% footnote(symbol="% is the percentages of the verified participants among total active recruits per campaign type in each site")

knitr::kable(table10_Tm_recr_sign[,c(1,2,4:10)],digits=2,caption = 'Table 12. Time from Active Recruitment to Sign In in Days', row.names=FALSE, booktabs = T,align=c("l", "c", "c","c", "c", "c", "c", "c", "c")) %>% kable_styling(latex_options = "scale_down") %>% footnote(general="excluding participants whose sign in time was before active recruitment time")

knitr::kable(table11_Tm_signin_cst.act1,digits=2,caption = 'Table 13a. Time from Sign In to Consent in Days among Active Recruits',booktabs=T, row.names=FALSE, align=c("l", "c", "c","c", "c", "c", "c", "c", "c")) %>% kable_styling(latex_options = "scale_down")
#%>% footnote(general="excluding participants who have not yet been verified and whose sign-in time were before recruitment time")

knitr::kable(table11_Tm_signin_cst.pas,digits=2,caption = 'Table 13b. Time from Sign In to Consent in Days among Passive Recruits',booktabs=T, row.names=FALSE, align=c("l", "c", "c","c", "c", "c", "c", "c", "c")) %>% kable_styling(latex_options = "scale_down")

knitr::kable(table12_Tm_consent_prof.act1[,c(1,2,4:11)],digits=2,caption = 'Table 14a. Time from Consent to User Profile Submitted in Days among Active Recruits', row.names=FALSE, booktabs = T, align=c("l", "c", "c","c", "c", "c", "c", "c", "c"))  %>% kable_styling(latex_options = "scale_down") 
 

knitr::kable(table12_Tm_consent_prof.pas[,c(1,2,4:11)],digits=2,caption = 'Table 14b. Time from Consent to User Profile Submitted in Days among Passive Recruits', row.names=FALSE, booktabs = T, align=c("l", "c", "c","c", "c", "c", "c", "c", "c"))  %>% kable_styling(latex_options = "scale_down")

knitr::kable(table13_Tm_prof_verif.act[,c(1,2,3,5:11)],digits=2,caption = 'Table 15a. Time from User Profile Submitted to Verification Complete in Days among Active Recruits', row.names=FALSE, booktabs = T, align=c("l", "c", "c","c", "c", "c", "c", "c", "c")) %>% kable_styling(latex_options = "scale_down")%>% footnote(general="excluding participants whose UP submitted time were before their verification time")

knitr::kable(table13_Tm_prof_verif.pas[,c(1,2,4:11)],digits=2,caption = 'Table 15b. Time from User Profile Submitted to Verification Complete in Days among Passive Recruits', row.names=FALSE, booktabs = T, align=c("l", "c", "c","c", "c", "c", "c", "c", "c")) %>% kable_styling(latex_options = "scale_down")

knitr::kable(table14_Tm_enroll_verif.act[,c(1,2,3,5:11)],digits=2,caption = 'Table 16a. Total Enrollment Time in Days among Active Participants', row.names=FALSE, booktabs = T, align=c("l", "c", "c","c", "c", "c", "c", "c", "c")) %>% kable_styling(latex_options = "scale_down")%>%landscape()%>% footnote(general="excluding participants whose enrollment were before their verification completion time")

knitr::kable(table14_Tm_enroll_verif.pas[,c(1,12,2,4:10)],digits=2,caption = 'Table 16b. Total Enrollment Time from Sign-In to Verification in Days among Passive Participants', row.names=FALSE, booktabs = T, align=c("l", "c", "c","c", "c", "c", "c", "c", "c")) %>% kable_styling(latex_options = "scale_down")%>%landscape() 

signin_methods1 %>%  kable_styling(latex_options = "scale_down")%>%landscape()
 
knitr::kable(Table16b_awaresreasons[,c(14,3:12),],caption = 'Table 18. How Recruits Heard About Study', row.names=FALSE, align=c("l", "c", "c","c", "c", "c", "c", "c", "c", "c","c","c")) %>% kable_styling(latex_options = "scale_down")%>% landscape() %>% footnote(number="%: percentage of each item in awareness among recruits within each site")
...
#Till now no incentive payments have been issued to the eligible participants in UoC site
 ...
 chica_ince %>% kable_styling(latex_options = "scale_down") %>% footnote(general="Chicago has not sent incentive data yet to CCC")
 chia_issue_refused %>% footnote(general="Chicago has not sent incentive data yet to CCC")
 ...
 knitr::kable(table56_Tm_IncentIssued, caption ='Table 21. Time From Incentive Eligible to Incentive Issued by NORC in Days', row.names=FALSE, digits=2, booktabs=T, align=c("l", "c", "c","c", "c", "c", "c", "c", "c"))  %>% kable_styling(latex_options = "scale_down")%>% landscape() %>% footnote(general='for all sites except Chicago')
...
###Tables on refusal and withdrawal, baseline activity: biospecimen donation, Survey modules
 ...
 knitr::kable(refuse.withdrawls_w1[,c(1,2,8,9,3,4,6)],caption='Table 22. Refusals and Withdrawals Among All Consented Participants', row.names=FALSE, align=c("l","c","c","c","c","c","c","c"),col.names = c("Site","Refusal of Any Study Activity", "Refusal of All Study Activities","Revoked HIPAA Authorization", "Withdrew Consent", "Data Destruction", "Total Consented Participants"), format="latex", booktabs = TRUE) %>% kable_styling(latex_options = "scale_down") %>% landscape()
 
 knitr::kable(ref_wd_reasons_all1[,c(12,2:11)],caption='Table 23. Reasons for Refusal or Withdrawal Among Those that Requested',caption.short = "Reasons for Refusal/Withdrawl.", row.names=FALSE,align=c("l","c","c","c","c","c","c","c","c","c")) %>% kable_styling(latex_options = c("scale_down")) %>%landscape() %>% footnote(general = 'total counts of refusal and withdrawal reasons are not equal to the total numbers of refusal and withdrawal cases')

Modbiospe.All%>% kable_styling(latex_options = c("scale_down")) %>% landscape()

for (i in 1:9){print(Modbiospe.site[[i]]  %>% kable_styling(latex_options = c("scale_down")) %>%  landscape())}

knitr::kable(Table_cmplt,caption='Table 34. Number of Verified Participants, Complete Participants, Iinvitations, Opt Outs, Withdrawals',row.names=F,booktabs=TRUE) %>% kable_styling(latex_options = "scale_down")
...
verified_expect.plot <- ggplot(verifed_expected,aes(x=Site))+
   ...
   geom_text(aes(y=Expected_Verified_n,label=Expected_Verified_n),size = 3,color="black",hjust=-0.5, vjust = -0.5 ,position = position_dodge(width=1))+
   ...
   scale_y_continuous(name="Individuals, N", breaks=c(500,1000,1500,2000,2500,3000,3500,4000,4500,5000),
...
response_rate.plot<-   ggplot(recrui_wk_verified,aes(x=as.Date(recruit.week.date))) + 
     ...
     scale_y_continuous(name="# Recruitments",  #, breaks=c(50,500,1000,1500,850,1050,1250,1450,1650,1850,2150,600,650,700,750,800,850,900,950,5000)
                        
                        sec.axis = sec_axis( ~ . /500 , name = "Cumulative Response Rate (%)")) +
     coord_cartesian(ylim =c(45,6000)) +
     scale_x_date(limit=c(as.Date("2022-01-01", origin = lubridate::origin),as.Date("2023-01-31", origin = lubridate::origin)),name="Date of Rercruitment",date_labels = "%m/%d/%Y", date_breaks = "2 weeks")  +
     ggtitle("Number Overall Verified & Response Rate Over Time till Jan.31, 2023")+
     theme( axis.text.x=element_text(angle=60, hjust=1), axis.title.x = element_text( size = 14, face = "bold") ,
            axis.title.y = element_text(size = 14, face = "bold",color="black"),
...
fig4<- ggplot(veri_wk_eligble,aes(x=as.Date(verified.week.date, origin = lubridate::origin))) + 
     ...
     scale_y_continuous(name="# Verified Participants",  #, breaks=c(50,500,1000,1500,850,1050,1250,1450,1650,1850,2150,600,650,700,750,800,850,900,950,5000)
                        ...
                        sec.axis = sec_axis( ~ . , name = "Cumulative Complete Participants")) +
     coord_cartesian(ylim =c(45,6000)) +
     scale_x_date(limit=c(as.Date("2022-01-01", origin = lubridate::origin),as.Date("2023-01-31", origin = lubridate::origin)),name="Date of Rercruitment",date_labels = "%m/%d/%Y", date_breaks = "1 month")  +
     ggtitle("Number Verified Participants & Complete Participants Over Time of All Sites till Jan.31, 2023")+
     theme( axis.text.x=element_text(angle=60, hjust=1), axis.title.x = element_text( size = 14, face = "bold") ,
            axis.title.y = element_text(size = 14, face = "bold",color="black"),
            axis.title.y.right = element_text(size=14,face="bold",color="red"),
            # Remove panel border
...
for (i in 1:9){
 ...
 figure <- ggplot(sub,aes(x=as.Date(verified.week.date, origin = lubridate::origin))) + 
    ...
    scale_y_continuous(name="# Verified Participants",  
     ...
     scale_x_date(limit=c(as.Date("2022-01-01", origin = lubridate::origin),as.Date("2023-01-31", origin = lubridate::origin)),name="Date of Rercruitment",date_labels = "%m/%d/%Y", date_breaks = "1 month")  +
     ggtitle(paste0("Number Verified Participants & Complete Participants ", site," Over Time till Jan.31, 2023"))+
...
```{r plots,eval=TRUE,echo=FALSE,results='asis'}



  site <- levels(veri_wk_eligble.site$site)
  sub <- veri_wk_eligble.site[which(veri_wk_eligble.site$site==site),]
  max <- max(sub$total_verified.cum)
  
 verified_eligible<- function(data,max){
   ggplot(data,aes(x=as.Date(verified.week.date, origin = lubridate::origin))) + 
     geom_area(aes(y=total_eligible.cum),fill="lightblue") +  
     geom_line(aes(y=total_verified.cum), color="red")+ 
    scale_y_continuous(name="# Verified Participants",  
                        sec.axis = sec_axis( ~ . , name = "Cumulative Complete Participants")) +
...
knitr::opts_chunk$set(warning = FALSE, message = FALSE,results='asis') 
 HP <- veri_wk_eligble.site[which(veri_wk_eligble.site$site==site[1]),] 
 max <- max(HP$total_verified.cum) 
HP<-verified_eligible(HP,max)

 HFHS <-  veri_wk_eligble.site[which(veri_wk_eligble.site$site==site[2]),] 
 max <- max(HFHS$total_verified.cum)
 HFHS<-verified_eligible(HFHS,max)
 
 MFH <- veri_wk_eligble.site[which(veri_wk_eligble.site$site==site[3]),] 
 max <- max(MFH$total_verified.cum)
 MFH<-verified_eligible(MFH,max) 

 SF <-  veri_wk_eligble.site[which(veri_wk_eligble.site$site==site[4]),] 
 max <- max(SF$total_verified.cum)
 SF<-verified_eligible(SF,max)
 
 UOC <- veri_wk_eligble.site[which(veri_wk_eligble.site$site==site[5]),] 
 max <- max(UOC$total_verified.cum)
 UOC<-verified_eligible(UOC,max)

```

```{r,echo=FALSE,include=FALSE,results='asis'}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
 KPCO <-  veri_wk_eligble.site[which(veri_wk_eligble.site$site==site[6]),] 
 max <- max(KPCO$total_verified.cum)
 KPCO <-verified_eligible(KPCO,max)
 
 KPGA <- veri_wk_eligble.site[which(veri_wk_eligble.site$site==site[7]),] 
 max <- max(KPGA$total_verified.cum)
 KPGA <-verified_eligible(KPGA,max) 

 KPHI <-  veri_wk_eligble.site[which(veri_wk_eligble.site$site==site[8]),] 
 max <- max(KPHI$total_verified.cum)
 KPHI <- verified_eligible(KPHI,max)
...
```{r, eval=TRUE,echo=FALSE,fig.cap='Figure 4.1', cache = TRUE}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
 HP <- veri_wk_eligble.site[which(veri_wk_eligble.site$site==site[1]),] 
 max <- max(HP$total_verified.cum) 
 verified_eligible(HP,max)
#HP
```

```{r, eval=TRUE,echo=FALSE,fig.cap='Figure 4.2',cache=TRUE}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
 HFHS <-  veri_wk_eligble.site[which(veri_wk_eligble.site$site==site[2]),] 
 max <- max(HFHS$total_verified.cum)
 verified_eligible(HFHS,max)
 
HFHS
```

```{r, eval=TRUE,echo=FALSE,fig.cap='Figure 4.3',cache=TRUE}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
 MFH <- veri_wk_eligble.site[which(veri_wk_eligble.site$site==site[3]),] 
 max <- max(MFH$total_verified.cum)
 verified_eligible(MFH,max) 
#MFH
```

```{r, eval=TRUE,echo=FALSE,fig.cap='Figure 4.4',cache=TRUE}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
...
```{r,eval=TRUE,echo=FALSE,fig.cap='Figure 4.5',cache=TRUE}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
...
```{r, eval=TRUE,echo=FALSE,fig.cap='Figure 4.6',cache=TRUE}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
KPCO
```

```{r, eval=TRUE,echo=FALSE,fig.cap='Figure 4.7',cache=TRUE}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
KPGA
```

```{r, eval=TRUE,echo=FALSE,fig.cap='Figure 4.8',cache=TRUE}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
KPHI

```

```{r, eval=TRUE,echo=FALSE,fig.cap='Figure 4.9',cache=TRUE}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
 KPNW <-  veri_wk_eligble.site[which(veri_wk_eligble.site$site==site[9]),] 
 max <- max(KPNW$total_verified.cum)
 KPNW <-verified_eligible(KPNW,max)
...
# # write.csv(ca_hist,paste("~/Documents/Connect_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/prod_previous_cancer_cases_profile_complete_",currentDate,".csv",sep=""),row.names = F,na="")
# 
...
# these variables don't exist in the current data: Columns `d_299274441_d_457532784`, `d_524352591_d_902332801`, `d_764403541`, `d_974100830`, and `d_672401635` `d_297147359_d_366295286`, `d_719222792_d_993855378`, `d_603278289_d_821930940`, `d_175506524_d_175506524`, `d_175506524_d_905787778`, `d_974100830` and `d_672401635` don't exist.
  refwd.main1 <- c("d_297147359_d_297147359","d_719222792_d_719222792","d_603278289_d_603278289","d_175506524_d_175506524","d_299274441_d_299274441","d_524352591_d_524352591","d_726389747","d_123868967","d_113579866","d_153713899","d_715390138")
  refwd_date1 <- c("d_657475009","d_982105411","d_390198398","d_808663245","d_262357957","d_711256590","d_161366008","d_614264509","d_664453818","d_269050420","d_659990606","d_613641698","d_119449326")
# 
...
# bq <- bq_project_query(project, query="SELECT token,Connect_ID, d_821247024,d_827220437,d_297147359_d_297147359,
# d_719222792_d_719222792,d_603278289_d_603278289,d_299274441_d_299274441,
# d_524352591_d_524352591,d_726389747,d_123868967,d_113579866,d_153713899,d_613641698,d_119449326,d_715390138,
#                 d_685002411_d_217367618,d_685002411_d_994064239,d_685002411_d_194410742,d_685002411_d_949501163,
#                  d_685002411_d_277479354,d_685002411_d_867203506,d_685002411_d_352996056,d_906417725,
#                  d_773707518,d_831041022,d_987563196,d_747006172,d_657475009,d_982105411,d_390198398,d_808663245,
#                  d_262357957,d_711256590,d_161366008,d_614264509,d_664453818,d_269050420,d_175506524.d_175506524,
#                  d_659990606,d_150818546,d_624030581,d_285488731,d_596510649,d_866089092,d_990579614,d_131458944
#                  d_919699172,d_141450621,d_576083042,d_431428747,d_121430614,d_523768810,d_639172801,d_175732191,
...
#               FROM `nih-nci-dceg-connect-prod-6d04.FlatConnect.participants_JP` WHERE d_512820379 != '180583933'")
# rewd <-  bq_table_download(bq,bigint="integer64")
 # rd <- bq_project_query(project, query="SELECT token,d_175506524.d_175506524 FROM `nih-nci-dceg-connect-prod-6d04.Connect.participants` WHERE d_821247024=197316935 or d_230663853 = 353358909")
 # 
 # refwd_175506524 <-  bq_table_download(rd,bigint="integer64")
rewd$refwd.cases=0
# refwd_cases <- list()
# # for (i in 1:13){
# # 
# #   case <- rewd[,c("token","Connect_ID","d_827220437",refwd_type1[i],refwd_date1[i])]
# #   colnames(case) <-c("token","Connect_ID","d_827220437","refwd_yes","refwd_dates")
...
# refwd.cases1 %>% refwd.cases1[refwd.cases==104430631] <- NA
...
# 
#  refwd.cases <- rewd[which(rewd$refwd.cases>0),c("token","Connect_ID","d_827220437","verified","recrt_steps_v1r0",refwd_type1,refwd_date1,refwd.main1,refwd_reasons)]
# 
###to combine the refusal types, and reasons individually
 # create a new column `x` with the three columns collapsed together

# refwd_yes$CID = sapply(strsplit(as.character(refwd_yes$refwd_type),"d_"),tail,1)
# refwd_yes$case.labels <- dd[which(refwd_yes$cases),"Variable Label"]
# refwd_yes <- merge(refwd_yes,dd[,c("CID","Variable Label")],by="CID", all.x=TRUE)
# 
#  refwd_yes_cases <- refwd_yes %>% select(token,Connect_ID,`Variable Label`) %>% group_by(token,Connect_ID) %>% mutate(case_labels=paste(`Variable Label`,collapse="|"))
# 
# # # remove the unnecessary columns
# # #data <- data[ , !( names( data ) %in% cols ) ]
#   write.csv(refwd.cases,paste("~/Documents/Connect_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/prod_refusal_withdrawl_cases_",currentDate,".csv",sep=""),row.names = F,na="")

  # #write.csv(refwd.cases,"~/Documents/Connect_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/prod_refusal_withdrawl_cases_01-17-2023.csv",row.names = F,na="")
# 
...
# # 
# #   
# #  #write.csv(duplicates,"~/Documents/Connect_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/prod_duplicates_01-17-2023.csv",row.names = F,na="")
# # ##incentive data
...
#write.csv(incentive_nonchi[,c("token","site","NORC.Payment.Eligibility","Eligible.for.Payment","Time.Payment.Eligible.for.Round")],"~/Documents/Connect_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/INcenctives_Weekly_Report_NonChicagoSites_01-23-2023.csv",row.names = F,na="")
...
# box.com remote file reference
# 
#  name        : Incentives_Weekly_Report_2023-01-03.csv 
#  file id     : 1105130583431 
#  version     : V1 
#  size        : 29 kB 
#  modified at : 2023-01-03 08:31:28 
#  created at  : 2023-01-03 08:31:28 
#  uploaded by : wuj12@nih.gov 
#  owned by    : brotzmanmj@nih.gov 
#  shared link : None 
# 
#  parent folder name :  NORC Incentive Eligible Reports_DCEG 
#  parent folder id   :  167284102961 
# box.com remote file reference /*data was downloaded 01/09/2023
# 
#  name        : Incentives_Weekly_Report_2023-01-09.csv 
#  file id     : 1110338114639 
#  version     : V1 
#  size        : 37 kB 
#  modified at : 2023-01-10 08:34:47 
#  created at  : 2023-01-10 08:34:47 
#  uploaded by : wuj12@nih.gov 
#  owned by    : brotzmanmj@nih.gov 
#  shared link : None 
# 
