...
This is the Daily report for Connect on Recruitment Progress from Invitation, through Opt-out, Sign-In, Consent, User Profile, and Verification. The Demographic information for the recruits is summarized by age, race/ethnicity, and sex. Response rate is calculated as the total number of participants gained from active recruitment both among all sites and for each site individually. The completion status of all verified participants is based on the completion of the four baseline survey modules and biospecimen donations.
...
All variables are stores as CIDs which can be tracked in the master dictionary: “https:// episphere.github.io/conceptGithubActions/aggregate.json” This report is for "`r format(Sys.Date(), '%d %B, %Y')`"
...
#saveRDS(recr_noinact_wl1,"~/Documents/CONNECT_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/recruit02222023.rds")
 ...
 data$verified <- factor(data$verified,exclude=NULL, levels=c("Not yet verified","Verified","Cannot be verified","Duplicate","Outreach timed out"))
  ...
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
    ...
    modify_header(stat_0 = "Total Passive Recruits \n With Profile Completion",
                ...
                stat_3="Cannot Be \n Verified",
                stat_5="Outreach \n Timed Out") %>%
    modify_caption("Table 8b. Passive Recruits Verification Status With Profile Completion") %>%
      as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
#  699625233	Autogenerated flag when User Profile submitted	User Profile submitted	RcrtUP_Submitted_v1r0  
    ...
    Table8a_opt_act <- data %>% filter(d_512820379==486306141) %>% select(site, Preconsent) %>% tbl_cross(
       row = site,
       col = Preconsent,
       label = list(Preconsent ~ "Pre-Consent Opt-Outs",site~"Site"),
       percent = "column",
       digits=c(0,2),
       margin_text="Total Active Recruits")
    ...
    Table8a_opt_act <-  Table8a_opt_act    %>%  
      modify_header(stat_0="Total Active \n Recruits") %>%
      modify_caption("Table 9.  Pre-Consent Opt-Outs Among Active Recruits") %>%
      as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
#Table 9.  Timing of Pre-consent Opt Out Among Active Recruits**") %>%	  
    precst_act <- data[which(data$d_512820379==486306141 &  data$state_d_158291096 == 353358909),] %>% 
      mutate(CSSigned = case_when(d_230663853 ==353358909 & d_919254129 == 104430631 ~ "Signed-In, no Consent",
                                  d_230663853 == 104430631 & d_919254129 == 104430631 ~ "Never Signed-In",
                                  d_230663853 ==353358909 & d_919254129 == 353358909 ~"Signed-In, Consent Submitted")) %>%
   ...
   table_precst_act$`Signed-In, no Consent` <-ifelse(table_precst_act$site != "Total" & is.na(table_precst_act$`Signed-In, no Consent`), 0,
                                               ifelse(table_precst_act$site !="Total" & table_precst_act$`Signed-In, no Consent` >=0, table_precst_act$`Signed-In, no Consent`, sum(precst_act_w$`Signed-In, no Consent`,na.rm=TRUE)))  

    table_precst_act$`Signed-In, Consent Submitted` <-ifelse(table_precst_act$site != "Total" & is.na(table_precst_act$`Signed-In, Consent Submitted`), 0,
                                               ifelse(table_precst_act$site !="Total" & table_precst_act$`Signed-In, Consent Submitted` >=0, table_precst_act$`Signed-In, Consent Submitted`, sum(precst_act_w$`Signed-In, Consent Submitted`,na.rm=TRUE))) 
...
table_precst_act$Npct.Signed.In.Consent <-  paste0(table_precst_act$`Signed-In, Consent Submitted`," (", round(100*table_precst_act$`Signed-In, Consent Submitted`/table_precst_act$Frequency, 2)," %)")
...
#table_precst_act$site <- ifelse(table_precst_act$site=="X  Total", "Total", table_precst_act$site)
 ...
 Table9_precst_act <- table_precst_act[,c(1,9:11,2)]
 colnames(Table9_precst_act) <- c("Site","Never Signed-In","Signed-In, no Consent","Signed-In, Consent Submitted","Total Active Recruits")

 data$CSSigned <- factor(data$CSSigned, levels=c("Never Signed-In","Signed-In, no Consent","Signed-In, Consent Submitted"))
    Table9_precst_act1 <- data %>% dplyr::filter(d_512820379==486306141& state_d_158291096 == 353358909) %>% 
      dplyr:: select(site, CSSigned) %>% tbl_cross(
        row = site,
        col = CSSigned,
        label = list(CSSigned ~ "Pre-consent After Opt-Outs",site~"Site"),
        percent = "row",
        digits = c(0,2),
        missing = "ifany",
        missing_text = "0 (0 %)",
        margin_text = "Total Pre-consent After Opt-Outs") 
 ...
 Table9_precst_act1 <- Table9_precst_act1 %>%
      ...
      modify_header(stat_0="Total Pre-consent \n After Opt-Outs",
                    ...
                    stat_2 = "Signed-In,\n No Consent",
                    stat_3 = "Opt Out \n Unknown") %>%
      modify_caption("Table 10.  Timing of Pre-Consent Opt-Outs Among Active Recruits") %>%
      as_kable_extra(escape = FALSE, addtl_fmt = TRUE) 
...
#Table 10.  Verification Status Among Those That Have Submitted User Profile
 ...
 Table10_profile <- Table10_profile%>%
      ...
      modify_header(stat_0 = "Total Submitted \n User Profile",
                ...
                stat_3="Cannot Be \n Verified",
                stat_5="Outreach \n Timed Out") %>%
      modify_caption("Table 11. Verification Status among Recruits That Have Submitted User Profile") %>%
      as_kable_extra(escape = FALSE, addtl_fmt = TRUE) 
...
#Table 16. Age of All Records
   ...
   table16_ageall <- table16_ageall%>%
     ...
     #italicize_levels() %>% 
     modify_header() %>%
     modify_caption("Table 17. Age of All Active and Passive Recruits") %>%
     as_kable_extra(escape = FALSE, addtl_fmt = TRUE) 
#Table 17. Race of All Records
   ...
   table17_raceall <- table17_raceall %>%
     ...
     #italicize_levels() %>% 
     modify_header() %>%
     modify_caption("Table 18. Race of All Active and Passive Recruits") %>%
     as_kable_extra(escape = FALSE, addtl_fmt = TRUE)   
...
#Table 18. Sex of All Records
   ...
   table18_sexall <- table18_sexall%>%
     ...
     modify_caption("Table 19. Sex of All Active and Passive Recruits") %>%
     as_kable_extra(escape = FALSE, addtl_fmt = TRUE)    
...
#Table 19. All Records by Site
   ...
   table19_recrall <- table19_recrall %>%
     ...
     #italicize_levels() %>% 
     modify_header() %>%
     modify_caption("Table 19. All Recruits by Site") %>%
     as_kable_extra(escape = FALSE, addtl_fmt = TRUE) 
...
# the total response rate: where %=(N Total Verified/N Total recruits)*100   
  ...
  active_recruits$total <- as.numeric(gsub(",", "", active_recruits$total))
   ...
   colnames(Table25_response.rate) <- gsub(".R"," R",colnames(Table25_response.rate))
 ...
 #Table 21.Completed Baseline Survey and Sample among Verified Participants and Response Ratio
  ...
  table21_surveycomplt <- table21_surveycomplt %>%
    ...
    #italicize_levels() %>% 
    modify_header(stat_0 = "Total Verified \n Participants") %>%
    modify_caption("Table 22. Completion of Baseline Survey and All Three Baseline Samples Among Verified Participants") %>%
    as_kable_extra(escape = FALSE, addtl_fmt = TRUE) 
...
# recr_process_long <- recr_process_long %>% dplyr::mutate(stage_new = case_when(recr_Process_v1r0 == 104430631 & stage ==1  ~ 0,
...
recr_count$stage <- factor(recr_count$stage_new,exclude=NULL)
recr_count$site <- factor(recr_count$d_827220437,exclude=NULL)
recr_count$site <- droplevels(recr_count$site)
colnames(recr_count)
recr_count$site <- factor(recr_count$site,exclude=NULL,
                    levels=c("HealthPartners", "Henry Ford Health System","Marshfield Clinic Health System",
                             "Sanford Health", "University of Chicago Medicine","Kaiser Permanente Colorado",
                             "Kaiser Permanente Georgia","Kaiser Permanente Hawaii","Kaiser Permanente Northwest"))

#recr_step <- reshape(recr_count,v.names="count",  idvar = "site", timevar = "stage_new",direction = "wide")
recr_count <- recr_count[order(recr_count$site),]
recr_count1 <- NULL
 for(stage in unique(recr_count$stage)){
   
     recr_count1 <- recr_count %>% filter(stage == stage) %>% arrange(factor(site)) %>% mutate(cum.count=cumsum(count),
                                                                                 label.count=cumsum(count)-0.1*count)
 }
...
recr_count2 <- recr_count1[which(recr_count1$stage_new >0 ),]
recr_count2$site <- factor(recr_count2$site,exclude=NULL,
                    levels=c("HealthPartners", "Henry Ford Health System","Marshfield Clinic Health System",
                             "Sanford Health", "University of Chicago Medicine","Kaiser Permanente Colorado",
                             "Kaiser Permanente Georgia","Kaiser Permanente Hawaii","Kaiser Permanente Northwest"))
recr_count2 <- recr_count2[order(recr_count2$site),]


mycolors <- c("#D53E4F","#F46D43","#FDAE61","#FEE08B","#E6F598", "#ABDDA4", "#66C2A5", "#3288BD", "#5E4FA2")

names(mycolors) <- levels(recr_count2$site)
...
Fig1 <-  ggplot(recr_count2, aes(x = stage, y = count,fill=fct_rev(site))) + 
  geom_bar(stat="identity",na.rm=TRUE)+ 
  #stat_summary(fun.y = sum, aes(label = ..y.., group = stage), geom = "text",vjust = -.2)+
  geom_text(data=totals, aes(x=stage, label=total, y=total), vjust = -.2)+
...
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
  ...
  UP_active_verified$site <- droplevels(UP_active_verified$site)
  names(mycolors) <- levels(UP_active_verified$site)
...
Fig2 <- ggplot(UP_active_verified, aes(x = verified, y = count,fill=fct_rev(site))) + 
  ...
  ggtitle("Fig 2.  Verification Status Among Active Recruits With Profile Completed") + 
  theme(panel.background = element_blank(),
...
```{r, include=FALSE,fig.cap='Figure 3'}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
...
#c("#D53E4F","#F46D43","#FDAE61","#FEE08B","#E6F598", "#ABDDA4", "#66C2A5", "#3288BD", "#5E4FA2")
  UP_passive_verified$site <- droplevels(UP_passive_verified$site)
names(mycolors) <- levels(UP_passive_verified$site)
...
Fig3 <- ggplot(UP_passive_verified, aes(x = verified, y = count,fill=fct_rev(site))) + 
  ...
  stat_summary(fun.y = sum, aes(label = ..y.., group = verified), geom = "text",vjust = -.2) +
  ggtitle("Fig 3. Verification Status Among Passive Recruits With Profile Completed") + 
  theme(panel.background = element_blank(),
        legend.title = element_text(size=7),
        axis.line = element_line(size=0.2),
...
```{r, eval=TRUE,include=FALSE,fig.cap='Figure 4'}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
Age_verified<- NULL
...
Age_verified <- Age_verified %>% group_by(age,site) %>% dplyr::summarise(count=n()) %>% 
  arrange(age,site) %>% mutate(cum.count=cumsum(count),
         label.count=cumsum(count)-0.1*count)
names(mycolors) <- levels(Age_verified$site)
...
```{r, eval=TRUE,include=FALSE,fig.cap='Figure 5'}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
...
#c("#D53E4F","#F46D43","#FDAE61","#FEE08B","#E6F598", "#ABDDA4", "#66C2A5", "#3288BD", "#5E4FA2")
names(mycolors) <- levels(sex$site)
...
```{r, eval=TRUE,include=FALSE,fig.cap='Figure 6'}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
 #Fig6. "Race of Verified Participants"
  ...
  names(mycolors) <- levels(race$site)
...
```{r, eval=TRUE,include=FALSE,fig.cap='Figure 7'}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
 #Fig7. "Completion of Initial Survey among Verified Participants"
  ...
  data$basesvcomplt <- factor(data$basesvcomplt, levels=c("All","2 or 3 sections", "BOH only","None"))
  names(mycolors) <- levels(Survey$site)
...
```{r, eval=TRUE,include=FALSE,fig.cap='Figure 8'}
knitr::opts_chunk$set(warning = FALSE, message = FALSE) 
#Fig8. "Completion of SSN among Verified Participants as of 05172022"
  ...
  #values= c("#D53E4F","#F46D43","#FDAE61","#FEE08B","#E6F598", "#ABDDA4", "#66C2A5", "#3288BD", "#5E4FA2")
  names(mycolors) <- levels(SSN$site)
...
knitr::kable(recruittypetotal[,c(4,1:3)],format.args = list(big.mark = ","), caption = 'Table 1.  Total Number of Active and Passive recruits', row.names=FALSE, align=c("l", "c", "c", "c")) %>% kable_styling(latex_options = "HOLD_position") %>% footnote(general = 'Not Active records are excluded from this report',general_title = "Note: ", footnote_as_chunk = T)

colnames(cumulative_all)[2] <- "Total Recruits"
knitr::kable(cumulative_all[,c(1,3:7,2)], caption = 'Table 2. Cumulative Status All Recruits', row.names=FALSE, align=c("l", "c", "c", "c","c", "c","c"),format.args = list(big.mark = ",")) %>% kable_styling(latex_options = "HOLD_position") %>% kable_styling(latex_options = "scale_down")

colnames(cumulative_active)[8] <- "Total Active Recruits"
knitr::kable(cumulative_active[,c(1,9:13,8)], caption = 'Table 3. Cumulative Status for Active Recruits Only', row.names=FALSE, align=c("l", "c", "c", "c", "c","c")) %>% kable_styling(latex_options = "HOLD_position") %>% kable_styling(latex_options = "scale_down")

colnames(cumulative_passive)[8] <- "Total Passive Recruits"
knitr::kable(cumulative_passive[,c(1,9:13,8)], caption = 'Table 4. Cumulative Status for Passive Recruits Only',  row.names=FALSE, align=c("l","c","c","c","c","l")) %>% kable_styling(latex_options = "HOLD_position") %>% kable_styling(latex_options = "scale_down")
...
Table8_profilecomplete %>% kable_styling(latex_options = "scale_down")%>% footnote(general = 'The Active duplicates are excluded from Active Recruits',general_title = "Note: ", footnote_as_chunk = T)
...
knitr::kable(Table25_response.rate, caption='Table 21. Verified Participants and Response Ratio based on Number of Active Recruits', row.names=FALSE, align=c("l","c", "c", "c"))
