---
title: "CCC_Consolidated_Weekly_Report"
author: "JingWU"
...
This Consolidated weekly report is a summary with tables and plots for the Connect Recruitment Progress by week from Invitation, through Opt-out, Sign-In, Consent, User Profile, and Verification. The Demographic information for the recruits is summarized by age, race/ethnicity, and sex. Response rate is calculated as the total number of participants gained from active recruitment both among all sites and for each site individually. The completion status of all verified participants is based on the completion of the four baseline survey modules and biospecimen donations.
...
All variables are stores as CIDs which can be tracked in the master dictionary: “https:// episphere.github.io/conceptGithubActions/aggregate.json” 


This CCC weekly report contains the Connect Recruitment workflow including, demographics, incentives, survey and biospecimen completion and refusal and withdrawals for active and passive recruits and verified participants on `r format(Sys.Date(), '%d %B, %Y')`.Atypical records under investigation are not included in the metrics tables and may result in Total columns that do not add up to 100%
...
# to check variables in recr_noinact_wl1
 ...
 # recr_noinact_wl1 <- readRDS("~/Documents/CONNECT_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/recruit2023-03-13.rds" )
...
data <- data[which(data$d_512820379 != 180583933 | data$d_821247024 ==197316935),]
data <- data[which (data$d_821247024 != 922622075 | data$d_512820379 !=486306141),] #248
data$site <- droplevels(data$site)
data<-data[order(data$site),]

data$recruit_type <- factor(data$recruit_type,levels=c("Active","Passive","Not Active But Verified"))
...
#Table 12 (Daily Table22). Baseline Biospecimen Collected among Verified Participants
    ...
    table12_biocol <- table12_biocol %>%
      ...
      #italicize_levels() %>% 
      modify_header(stat_0 = "Total Verified \n Participants") %>%
      modify_caption("Table 12. Baseline Biospecimen Collected among Verified Participants") %>%
      as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
#Daily Table 11. Age of Verified Participants
    table45_age <- data[which(data$d_821247024 == 197316935),] %>% 
...
#Daily Tabel 12. Race of Verified Participants
    ...
    table46_race <- data[which(data$d_821247024 == 197316935),] %>% select(site, race) %>% tbl_cross(
      row = site,
      col = race,
...
#Daily Table 13. Sex of Verified Participants
  data$sex <-factor(data$sex,levels=c("Female","Male", "Intersex or Other","Unknown"))
   ...
   table47 <- data[which(data$d_821247024 == 197316935),] %>% select(site, sex) %>% dplyr::arrange(site,sex) %>%
     tbl_cross(
      row = site,
      col = sex,
...
knitr::kable(cumulative_passive[,c(1,9:13,8)], caption = 'Table 6. Cumulative Status for Passive Recruits Only',  row.names=FALSE, align=c("l","c","c","c","c","c","c")) %>% kable_styling(latex_options = c("HOLD_position","scale_down")) %>% add_indent(c(1:9))
...
table9_verifiedtype %>% kable_styling(font_size = 9) %>% kable_styling(latex_options = "scale_down") %>%  landscape() 
...
refuse.withdrawls_w2 %>% 
  knitr::kable("latex",caption='Table 24. Refusals and Withdrawals Among All Consented Participants', row.names=FALSE, col.names = linebreak(names(refuse.withdrawls_w2), align = "l"), align=c("l","l","l","l","l","l","l"), booktabs = TRUE, linesep = "") %>% kable_styling(latex_options = c("scale_down"),full_width = F) %>% add_indent(c(1:9)) %>% footnote(general='Frequency % of the Total Comsented Participants by Site',  general_title = "Note:",footnote_as_chunk = T)
...
knitr::kable(optout1.sites[,c(9,4,8,7)],caption = 'Table 34. Total Number of Opt-Outs by Site',col.names=c("Site","Total Opt-Outs", "Percent %", "Total Active Recruits"), format.args = list(big.mark = ","), row.names=FALSE, align=c("l","c","c","c")) %>% add_indent(c(1:9)) %>% footnote(general='Frequency (%) of Overall Active Recruits of Connect',general_title = "Note:", footnote_as_chunk = T)
...
table45_age %>% kable_styling(latex_options = "scale_down")

table46_race

table47%>% kable_styling(font_size=8)

table48_ageall  %>% kable_styling(latex_options = "scale_down")

table49_raceall

table50_sexall %>% kable_styling(latex_options = "scale_down")
