...
#Start column for each split data frame
   ...
   # 
   #   recrbq[[i]] <- bq_table_download(tmp, bigint="integer64",n_max = Inf, page_size = 20)
   # }
...
# #  to check variables in recr_noinact_wl1
  ...
  #    saveRDS(recr_noinact_wl1,paste("~/Documents/CONNECT_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/recruit",currentDate,".rds", sep=""))
     recr_noinact_wl1 <- readRDS("~/Documents/CONNECT_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/recruit2023-07-31.rds" )
...
table38_Tm_IncentIssued.all <- incentive[complete.cases(incentive[,c("d_130371375_d_266600170_d_787567527","d_130371375_d_266600170_d_297462035")]),] %>% dplyr::filter(d_130371375_d_266600170_d_731498909 == 353358909 & d_130371375_d_266600170_d_648936790 == 353358909) %>% mutate(time=Tm_issued2eligible) %>%  dplyr::summarize('Incentives Issued'=n(),
                   min = min(time),
                   q1 = quantile(time, 0.25),
                   median = median(time),
                   mean = mean(time),sd=sd(time),
                   ...
                   pct95 = quantile(time, 0.95),
                   max = max(time))
table38_Tm_IncentIssued.all$site <- "Total"

table38_Tm_IncentIssued <- rbind(table38_Tm_IncentIssued,table38_Tm_IncentIssued.all)
...
table40_actpass <- pct_tb(active_frpass$site,active_frpass$passive_active)                                  
table40_actpass$`Total Active Verified Participants` <- table40_actpass$n_104430631+table40_actpass$n_353358909
colnames(table40_actpass)[which(colnames(table40_actpass)=="n_pct_353358909")] <- "Passive to Active"
table40_actpass$Site <- rownames(table40_actpass)

#Table 41 (weekly table 15.) Sign-In Mechanism Used for First Log On
...
signin_methods1 <- signin_methods1 %>%
  ...
  modify_caption("Table 41. Sign-In Mechanism for First Log on") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE) 
...
Table42_awaresreasons <- merge(dd[,c("CID","Variable Label")],awares1_transpose,by.x="CID",by.y="CID",all.y=TRUE)

Table42_awaresreasons$Awareness <- ifelse(is.na(Table42_awaresreasons$CID),"Total SignedIn",ifelse(Table42_awaresreasons$CID == 0, "No Response Sent",Table42_awaresreasons$`Variable Label`))
Table42_awaresreasons <- Table42_awaresreasons[order(Table42_awaresreasons$CID),]
...
Table42b_awaresreasons <- merge(dd[,c("CID","Variable Label")],awares2_transpose,by.x="CID",by.y="CID",all.y=TRUE)
Table42b_awaresreasons$Awareness <- ifelse(is.na(Table42b_awaresreasons$CID),"Total Recruits",ifelse(Table42b_awaresreasons$CID == 0, "No Response Sent",Table42b_awaresreasons$`Variable Label`))
Table42b_awaresreasons <- Table42b_awaresreasons[order(Table42b_awaresreasons$CID),]
...
#Table 43(Weekly table 8). Total Number of Active Recruits in Campaign
...
table43_Totalactivecamp <-  data %>% filter(d_512820379 == 486306141 & d_827220437 >0) %>% 
...
table43_Totalactivecamp[["table_body"]]$stat_0 <- sapply(strsplit(table43_Totalactivecamp[["table_body"]]$stat_0," "),"[",1) 

    
table43_Totalactivecamp <- table43_Totalactivecamp %>%
  #bold_labels() %>%
  #italicize_levels() %>% 
  modify_header( 
    ...
    'stat_9' = 'NA/missing') %>%
  modify_caption("Table 43. Total Number of Active Recruits in Campaign") %>%
  modify_footnote(all_stat_cols() ~ "Frequency (%) of Total Active Recruits") %>%
  as_kable_extra(escape = FALSE, addtl_fmt = TRUE)

table43_Totalactivecamp
...
#Table 44 (Weekly Table 9). Number ratio of the success campaign verified among recruits
...
for(site in unique(data[which(data$d_827220437>0),]$site)){
  ...
  total.veri_pct <- pct_tb(data[which(data$d_512820379==486306141),]$active_camptype,     data[which(data$d_512820379==486306141),]$Rcrt_Verified_v1r0)
       act.veri.camp <- as.data.frame(t(c(format(total.veri_pct$n_353358909[1:8],big.mark=","),paste0(round(100*total.veri_pct$pct_353358909[1:8],2)," %"))))
...
#Table 45 and 46 (equally weekly table 6 and 7): total number of opt-outs by site
...
optout1 <- optout[!empty_columns] #32 columns #34 columns(02/21/2023) #38 columns(05/30/2023)
...
Table47_optreasons <- merge(grab,optouts1_transpose,by.x="CID",by.y="CID",all.y=TRUE)
colnames(Table47_optreasons)[c(2,12)] <- c("Opt-Out Reasons","Total Opt-Outs")

Table47_optreasons$`Total Opt-Outs` <- sapply(strsplit(Table47_optreasons$`Total Opt-Outs`, " "), "[[",1)

Table47_optreasons[nrow(Table47_optreasons),2] <- "Total Opt-Outs"
#optout_reason1 <- colnames(optout1.sites)[c(6:17,19:32)]
...
##to remove the "Signed-In and Consent submittd" (unusual one, mistaken ones for QC)
 data$CSSigned <- factor(data$CSSigned, levels=c("Never Signed-In","Signed-In, no Consent"))
    Table46_precst_act1 <- data %>% dplyr::filter(d_512820379==486306141& state_d_158291096 == 353358909 & d_827220437>0) %>% 
    ...
    Table46_precst_act1[["table_body"]]$stat_0 <- sapply(strsplit(Table46_precst_act1[["table_body"]]$stat_0," "),"[",1) 

    
 Table46_precst_act1 <- Table46_precst_act1 %>%
      #bold_labels() %>%
      #italicize_levels() %>% 
      modify_header(stat_0="Total Pre-consent \nOpt-Outs",
                    stat_1 = "Never \n Signed-In",
                    stat_2 = "Signed-In,\n No Consent") %>%
      modify_caption("Table 46.  Timing of Pre-Consent Opt-Outs Among Active Recruits") %>%
      as_kable_extra(escape = FALSE, addtl_fmt = TRUE) 

 Table46_precst_act1 <- gsub("NA","0", Table46_precst_act1 )  
...
#Table 4. Current Status in Workflow Total
 data$recrt_steps_v1r0 <-factor(data$recrt_steps_v1r0,levels=c("Never Signed-In","Signed-in, No Consent","Consented, No Profile", "Profile, Verification Incomplete","Verification Complete"))
    ...
    work_flow_a <- work_flow_a%>%
      ...
      #italicize_levels() %>% 
      modify_header() %>%
      modify_caption("Table 48. Current Status in Workflow All Recruits") %>%
      as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
   ...
   work_flow_a  <- gsub("NA","0", work_flow_a )  
    ...
    work_flow_act <- work_flow_act%>%
      ...
      #italicize_levels() %>% 
      modify_header() %>%
      modify_caption("Table 49. Current Status in Workflow Active Recruits Only") %>%
      as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
    ...
    work_flow_pass <- work_flow_pass %>%
      ...
      #italicize_levels() %>% 
      modify_header() %>%
      modify_caption("Table 50. Current Status in Workflow Passive Recruits Only") %>%
      as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
table51_Tm_recr_sign1 <- recruit_timing %>% filter(d_512820379 == 486306141 & d_827220437 >0 & d_230663853==353358909 & tm_recr_signin>= 0 ) %>% 
...
table51_Tm_recr_sign.total <- recruit_timing %>% filter(d_512820379 == 486306141 & d_827220437 >0 & d_230663853==353358909 & tm_recr_signin>= 0 ) %>% 
  mutate(time = as.numeric(tm_recr_signin),
...
table51_Tm_recr_sign.total$site <- "Total"
table51_Tm_recr_sign <- rbind(table51_Tm_recr_sign1,table51_Tm_recr_sign.total)
colnames(table51_Tm_recr_sign)[2] <- "Total Active Sign-In"
table51_Tm_recr_sign$`Recruitment Type` <- "Active"
table51_Tm_recr_sign <- table51_Tm_recr_sign %>% dplyr::rename(Site=site)
...
table52_Tm_signin_cst <- recruit_timing %>% filter( d_827220437 >0 & 
...
table52_Tm_signin_cst$site <- "Total"
table52_Tm_signin_cst$recruitment_type <- ifelse(table52_Tm_signin_cst$d_512820379 == 486306141, "Active", ifelse(table52_Tm_signin_cst$d_512820379 ==854703046,"Passive", "Not Active"))

table52_Tm_signin_cst_errors <- recruit_timing%>% filter (d_827220437 >0 & 
                                                    d_230663853==353358909 & d_919254129== 353358909 & tm_signin_cst<0 )


table52_Tm_signin_cst.act <- recruit_timing %>% filter(d_512820379 == 486306141 & d_827220437 >0 & 
                                                    d_230663853==353358909 & d_919254129== 353358909 & tm_signin_cst>=0  ) %>% 
  mutate(time = as.numeric((tm_signin_cst)),
         site = droplevels(site)) %>% # Summary by group using dplyr, to make the time units as hours
  group_by(site) %>% 
...
table52_Tm_signin_cst.act <- rbind(table52_Tm_signin_cst.act,table52_Tm_signin_cst[which(table52_Tm_signin_cst$recruitment_type=="Active"),c(2:10)])
table52_Tm_signin_cst.act$order <- rownames(table52_Tm_signin_cst.act)

table52_Tm_signin_cst.act1 <- merge(signin_cst_type[which(signin_cst_type$x==486306141),], table52_Tm_signin_cst.act, by.x="y",by.y="site",all.y=TRUE)

table52_Tm_signin_cst.act1$recruitment_type <- "Active"
table52_Tm_signin_cst.act1 <- table52_Tm_signin_cst.act1[order(table52_Tm_signin_cst.act1$order),c(1,13,4:11)] ##remove the actual n of signin to consent
colnames(table52_Tm_signin_cst.act1)[1:3] <- c("Site","Recruitment Type","Number Consent")
table52_Tm_signin_cst.act1 <- table52_Tm_signin_cst.act1[order(as.numeric(rownames(table52_Tm_signin_cst.act1))),]


table52_Tm_signin_cst.pas <- recruit_timing %>% 
...
table52_Tm_signin_cst.pas <- rbind(table52_Tm_signin_cst.pas,table52_Tm_signin_cst[which(table52_Tm_signin_cst$recruitment_type=="Passive"),c(2:10)])
table52_Tm_signin_cst.pas$order <- as.numeric(rownames(table52_Tm_signin_cst.pas))

table52_Tm_signin_cst.pas <- merge(signin_cst_type[which(signin_cst_type$x==854703046),], table52_Tm_signin_cst.pas, by.x="y",by.y="site", all.y=TRUE)

table52_Tm_signin_cst.pas$recruitment_type <- "Passive"
table52_Tm_signin_cst.pas$Freq <- ifelse(is.na(table52_Tm_signin_cst.pas$Freq), sum(signin_cst_type[which(signin_cst_type$x==854703046),]$Freq),table52_Tm_signin_cst.pas$Freq )
table52_Tm_signin_cst.pas1 <- table52_Tm_signin_cst.pas[order(as.numeric(table52_Tm_signin_cst.pas$order)),c(1,13,3:11)]
colnames(table52_Tm_signin_cst.pas1)[1:3] <- c("Site","Recruitment Type","Total Consent")
...
table53_Tm_consent_prof <- recruit_timing %>% filter( d_827220437 >0 & d_230663853==353358909 &
...
table53_Tm_consent_prof$site <- "Total"
table53_Tm_consent_prof$recruitment_type <- ifelse(table53_Tm_consent_prof$d_512820379==486306141, "Active", ifelse(table53_Tm_consent_prof$d_512820379 ==854703046,"Passive", "Not Active"))

table53_Tm_consent_prof.act <- recruit_timing %>% filter(d_512820379 == 486306141 & d_827220437 >0 & d_230663853==353358909 &
                                         d_919254129== 353358909 & d_699625233 == 353358909 & !is.na(tm_consent_prof) ) %>% 
  mutate(time = as.numeric(tm_consent_prof),
         site = droplevels(site)) %>% # Summary by group using dplyr, to make the time units as hours
  group_by(site) %>% 
...
table53_Tm_consent_prof.act <- rbind(table53_Tm_consent_prof.act,table53_Tm_consent_prof[which(table53_Tm_consent_prof$recruitment_type=="Active"),c(2:10)])
table53_Tm_consent_prof.act$order <- rownames(table53_Tm_consent_prof.act)

table53_Tm_consent_prof.act1 <- merge(cst_prof[which(cst_prof$d_512820379 == 486306141),],table53_Tm_consent_prof.act,by.x="site",by.y="site", all.y=TRUE)


table53_Tm_consent_prof.act1$d_512820379  <- "Active" 
table53_Tm_consent_prof.act1<- table53_Tm_consent_prof.act1 %>% arrange(as.numeric(order))
colnames(table53_Tm_consent_prof.act1)[2] <- "Recruitment Type"
colnames(table53_Tm_consent_prof.act1)[4] <- "Number UP to Submit"
table53_Tm_consent_prof.act1 <- table53_Tm_consent_prof.act1 %>% dplyr::rename(Site=site)

table53_Tm_consent_prof.pas <- recruit_timing %>% filter(d_512820379 == 854703046 & d_827220437 >0 & d_230663853==353358909 &
                                   d_919254129== 353358909 & d_699625233 == 353358909 & !is.na(tm_consent_prof ) ) %>% 
  mutate(time = as.numeric(tm_consent_prof),
         site = droplevels(site)) %>% # Summary by group using dplyr, to make the time units as hours
  group_by(site) %>% 
...
table53_Tm_consent_prof.pas <- rbind(table53_Tm_consent_prof.pas,table53_Tm_consent_prof[which(table53_Tm_consent_prof$recruitment_type=="Passive"),c(2:10)])
table53_Tm_consent_prof.pas$order <- rownames(table53_Tm_consent_prof.pas)
table53_Tm_consent_prof.pas <- merge(cst_prof[which(cst_prof$d_512820379 == 854703046),],table53_Tm_consent_prof.pas,by.x="site",by.y="site", all.y=TRUE)

table53_Tm_consent_prof.pas$d_512820379 <- "Passive"
table53_Tm_consent_prof.pas<- table53_Tm_consent_prof.pas %>% arrange(as.numeric(order) )
colnames(table53_Tm_consent_prof.pas)[2] <- "Recruitment Type"
colnames(table53_Tm_consent_prof.pas)[4] <- "Number UP to Submit"
table53_Tm_consent_prof.pas <- table53_Tm_consent_prof.pas %>% dplyr::rename(Site=site)
...
table54_Tm_prof_verif <- recruit_timing %>% filter( d_827220437 >0 & d_230663853==353358909 & d_919254129== 353358909 & d_699625233 == 353358909 & d_821247024 != 875007964 & tm_prof_verif>0) %>% 
  ...
  dplyr::summarize('Number to Verification Completion' = n(),
                   ...
                   q3 = quantile(time, 0.75),
                   max = max(time))
table54_Tm_prof_verif$Site <- "Total"
table54_Tm_prof_verif$recruitment_type <- ifelse(table54_Tm_prof_verif$d_512820379==486306141, "Active", ifelse(table54_Tm_prof_verif$d_512820379 ==854703046,"Passive", "Not Active"))


table54_Tm_prof_verif.act <- recruit_timing %>% filter(d_512820379 == 486306141 & d_827220437 >0 & d_230663853==353358909 & d_919254129== 353358909 & d_699625233 == 353358909 & d_821247024 != 875007964 & tm_prof_verif>0 ) %>% 
  mutate(time = as.numeric(tm_prof_verif),
         Site = droplevels(site)) %>% # Summary by group using dplyr, to make the time units as hours
  group_by(Site) %>% 
  dplyr::summarize('Number to Verification Completion' = n(),
...
table54_Tm_prof_verif.act <- rbind(table54_Tm_prof_verif.act,table54_Tm_prof_verif[which(table54_Tm_prof_verif$recruitment_type=="Active"),c(2:10)])   
table54_Tm_prof_verif.act$`Recruitment Type` <- "Active"
#colnames(table13_Tm_prof_verif.act)[2] <- "Total Active UPs to Verification"
colnames(table54_Tm_prof_verif.act)[2] <- "Total UP Submitted"


table54_Tm_prof_verif.pas <- recruit_timing %>% 
...
table54_Tm_prof_verif.pas <- rbind(table54_Tm_prof_verif.pas,table54_Tm_prof_verif[which(table54_Tm_prof_verif$recruitment_type=="Passive"),c(2:10)]) 
table54_Tm_prof_verif.pas$order <- rownames(table54_Tm_prof_verif.pas)
table54_Tm_prof_verif.pas <- merge(prof_verif[which(prof_verif$d_512820379 == 854703046),],table54_Tm_prof_verif.pas,by.x="Site",by.y="Site", all.y=TRUE)

table54_Tm_prof_verif.pas$d_512820379 <- ifelse(table54_Tm_prof_verif.pas$Site=="Total","Passive",gsub("854703046","Passive",table54_Tm_prof_verif.pas$d_512820379))
table54_Tm_prof_verif.pas$n <- ifelse(table54_Tm_prof_verif.pas$Site=="Total", sum(prof_verif[which(prof_verif$d_512820379 == 854703046),]$n), table54_Tm_prof_verif.pas$n)
table54_Tm_prof_verif.pas<- table54_Tm_prof_verif.pas %>% arrange(as.numeric(order) )
colnames(table54_Tm_prof_verif.pas)[2] <- "Recruitment Type"
#colnames(table13_Tm_prof_verif.pas)[4] <- "Total Passive UPs to Verification"
colnames(table54_Tm_prof_verif.pas)[4] <- "Total UP submitted"
...
table55_Tm_enroll_verif <- recruit_timing[complete.cases(recruit_timing[,c("verified_time","recru_time")]),] %>% 
  ...
  dplyr::summarize('Number Enrollment'=n(),
                   ...
                   q3 = quantile(time, 0.75),
                   max = max(time))
table55_Tm_enroll_verif$site <- "Total"
table55_Tm_enroll_verif$recruitment_type <- ifelse(table55_Tm_enroll_verif$d_512820379==486306141, "Active", ifelse(table55_Tm_enroll_verif$d_512820379 ==854703046,"Passive", "Not Active"))


table55_Tm_enroll_verif.act <- recruit_timing[complete.cases(recruit_timing[,c("verified_time","recru_time")]),] %>% 
...
table55_Tm_enroll_verif.act$recruitment_type <- "Active"
table55_Tm_enroll_verif.act <- rbind(table55_Tm_enroll_verif.act,table55_Tm_enroll_verif[which(table55_Tm_enroll_verif$recruitment_type=="Active"),c(2:11)]) 

         
colnames(table55_Tm_enroll_verif.act)[1:2] <- c("Site","Total Verification Complete")

#passive from sign in time to verification time
table55_signin_verif.pas <- recruit_timing[complete.cases(recruit_timing[,c("verified_time","signin_time")]),] %>% 
...
table55_Tm_signin_verif.pas <- recruit_timing[complete.cases(recruit_timing[,c("verified_time","signin_time")]),] %>% 
  filter(d_512820379 == 854703046 & d_827220437 >0 & d_230663853==353358909 &
  ...
  dplyr::summarize('Number Verification Complete'=n(),
                   ...
                   q3 = quantile(time, 0.75),
                   max = max(time))
table55_Tm_signin_verif.pas$site <- "Total"

table55_Tm_enroll_verif.pas <- recruit_timing[complete.cases(recruit_timing[,c("verified_time","signin_time")]),] %>%   filter(d_512820379 == 854703046 & d_827220437 >0 & d_230663853==353358909 &
         d_919254129== 353358909 & d_699625233 ==353358909 & d_821247024 != 875007964 & as.numeric(difftime(verified_time,signin_time,units="days")) >=0 ) %>% 
  mutate(time = as.numeric(difftime(verified_time,signin_time,units="days")),
         site = droplevels(site)) %>% 
  group_by(site) %>% 
...
table55_Tm_enroll_verif.pas <- rbind(table55_Tm_enroll_verif.pas,table55_Tm_signin_verif.pas) 
table55_Tm_enroll_verif.pas$order <- as.numeric(rownames(table55_Tm_enroll_verif.pas))
table55_Tm_enroll_verif.pas <- merge(table55_signin_verif.pas,table55_Tm_enroll_verif.pas,by.x="site",by.y="site", all.y=TRUE)

table55_Tm_enroll_verif.pas$`Recruitment Type` <- "Passive"
table55_Tm_enroll_verif.pas <- table55_Tm_enroll_verif.pas %>% arrange(order) 
table55_Tm_enroll_verif.pas$`Total Verification Complete` <- ifelse(table55_Tm_enroll_verif.pas$site =="Total", sum(table55_signin_verif.pas$`Total Verification Complete`),table55_Tm_enroll_verif.pas$`Total Verification Complete` )
table55_Tm_enroll_verif.pas <- table55_Tm_enroll_verif.pas %>% dplyr::rename(Site=site)
#colnames(table14_Tm_enroll_verif.act)[2] <- "Recruitment Type"

#Daily Table 11. Age of Verified Participants
    table56_age <- data[which(data$d_821247024 == 197316935),] %>% mutate(site=droplevels(site)) %>%
    ...
    table56_age[["table_body"]]$stat_0 <- sapply(strsplit(table56_age[["table_body"]]$stat_0," "),"[",1) 
    
    table56_age <- table56_age%>%
           #bold_labels() %>%
           #italicize_levels() %>% 
           modify_header(stat_0 = "Total Verified \n Participants") %>%
           modify_caption("Table 56. Age of Verified Participants") %>%
           #modify_footnote(all_stat_cols() ~"Note: Based on de-identified data sent by sites") %>%
            as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
			
#Daily Tabel 12. Race of Verified Participants
    data$race <- factor(data$race, levels=c("White","Other","Unknown"))
    table57_race <- data[which(data$d_821247024 == 197316935),] %>% select(site, race) %>% 
     mutate(site=droplevels(site))  %>% tbl_cross(
      row = site,
      col = race,
    ...
    table57_race[["table_body"]]$stat_0 <- sapply(strsplit(table57_race[["table_body"]]$stat_0," "),"[",1) 
    
    table57_race <- table57_race %>%
      #bold_labels() %>%
      #italicize_levels() %>% 
      modify_header(stat_0 = "Total Verified \n Participants") %>%
      modify_caption("Table 57. Race of Verified Participants") %>%
      #modify_footnote(all_stat_cols() ~"Note: Based on de-identified data sent by sites") %>%
      as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
...
#Daily Table 13. Sex of Verified Participants
  data$sex <-factor(data$sex,levels=c("Female","Male", "Intersex or Other","Unknown"))
   ...
   table58 <- data[which(data$d_821247024 == 197316935),] %>% mutate(site=droplevels(site)) %>% select(site, sex) %>% dplyr::arrange(site,sex) %>%
     tbl_cross(
      row = site,
      col = sex,
   ...
   table58[["table_body"]]$stat_0 <- sapply(strsplit(table58[["table_body"]]$stat_0," "),"[",1)  
   table58 <- table58 %>%
      #bold_labels() %>%
      #italicize_levels() %>% 
      modify_header(stat_0 = "Total Verified \n Participants") %>%
      modify_caption("Table 58. Sex of Verified Participants") %>%
      #modify_footnote(all_stat_cols() ~"Note: Based on de-identified data sent by sites") %>%
      as_kable_extra(escape = FALSE, addtl_fmt = TRUE)
    

#Table 48. Daily Table 15 Age of All Records
   table59_ageall <- data[which(data$d_827220437>0),] %>% select(site, age) %>% tbl_cross(
     row = site,
     col = age,
     label = list(age~ "Age",site ~ "Site"),
     percent = "row",
   ...
   table59_ageall[["table_body"]]$stat_0 <- sapply(strsplit(table59_ageall[["table_body"]]$stat_0," "),"[",1)
    
   table59_ageall <- table59_ageall%>%
     #bold_labels() %>%
     #italicize_levels() %>% 
     modify_header() %>%
     modify_caption("Table 59. Age of All Active and Passive Recruits") %>%
     as_kable_extra(escape = FALSE, addtl_fmt = TRUE) 
   
#Daily Table 17. Race of All Records
   table60_raceall <- data[which(data$d_827220437>0),] %>% select(site, race) %>% tbl_cross(
     row = site,
     col = race,
     label = list(race~"Race",site ~ "Site"),
     percent = "row",
   ...
   table60_raceall[["table_body"]]$stat_0 <- sapply(strsplit(table60_raceall[["table_body"]]$stat_0," "),"[",1) 
    
   table60_raceall <- table60_raceall %>%
     #bold_labels() %>%
     #italicize_levels() %>% 
     modify_header() %>%
     modify_caption("Table 60. Race of All Active and Passive Recruits") %>%
     as_kable_extra(escape = FALSE, addtl_fmt = TRUE)   
    
#Table 61 (daily Table 18.) Sex of All Records
   table61_sexall <- data[which(data$d_827220437>0),] %>% select(site, sex) %>% 
   ...
   table61_sexall[["table_body"]]$stat_0 <- sapply(strsplit(table61_sexall[["table_body"]]$stat_0," "),"[",1) 
    
   table61_sexall <- table61_sexall%>%
     #bold_labels() %>%
     #italicize_levels() %>% 
     modify_header() %>%
     modify_caption("Table 61. Sex of All Active and Passive Recruits") %>%
     as_kable_extra(escape = FALSE, addtl_fmt = TRUE)   
...
fig4<- ggplot(veri_wk_eligble,aes(x=as.Date(verified.week.date, origin = lubridate::origin))) + 
     ...
     scale_x_date(limit=c(as.Date("2022-01-01", origin = lubridate::origin),currentDate),name="Date of Recruitment",expand=c(0,0),date_labels = "%m/%d/%Y", date_breaks = "2 weeks")  +
...
veri_wk_eligble.site$verified.week.date[is.na(veri_wk_eligble.site$verified.week.date)] <- recrstart.date + veri_wk_eligble.site$verified.week*7
...
for (i in 1:9){
 ...
 figure <- ggplot(sub,aes(x=as.Date(verified.week.date, origin = lubridate::origin))) + 
    ...
    scale_y_continuous(name="# Verified Participants",  breaks=breaks.site,
     ...
     scale_x_date(limit=c(as.Date("2022-01-01", origin = lubridate::origin),currentDate),expand=c(0,0),name="Date of Recruitment",date_labels = "%m/%d/%Y", date_breaks = "2 weeks")  +
...
tb_refwd %>% kable_styling(latex_options = c("scale_down"),full_width = F) %>% add_indent(c(1:9)) %>% footnote(general='% of the Total Consented Participants by Site',  general_title = "Note:",footnote_as_chunk = T)
...
#%>% footnote(general="University of Chicago Medicine has not sent incentive data yet to CCC",general_title = "Note: ",footnote_as_chunk = T)
 ...
 colnames(table38_Tm_IncentIssued)[1] <-"Site"
 colnames(table38_Tm_IncentIssued)[9:10] = c("90 %", "95 %")
 knitr::kable(table38_Tm_IncentIssued, caption ='Table 38. Time From Incentive Eligible to Incentive Issued by NORC in Days', row.names=FALSE, digits=2, booktabs=T, linesep = "", align=c("l", "c", "c","c", "c", "c", "c", "c", "c"))  %>% kable_styling(latex_options = "scale_down")%>% landscape() %>% add_indent(c(1:8)) %>% footnote(general='For all sites except Chicago',footnote_as_chunk = T)
```
...
knitr::kable(cancer.veri.up[,c(1,6,5,2,3,7)],caption = 'Table 39. Previous Cancer Reported in User Profile by Verification Status', row.names=FALSE,format.args = list(big.mark = ","), align=c("l","c","c","c","c","c"),booktabs = T, linesep = "",
...
colnames(table52_Tm_signin_cst.act1)[4:10] <- c("Min","Q1","Median","Mean","SD","Q3","Max")
knitr::kable(table52_Tm_signin_cst.act1,digits=2,format.args = list(big.mark = ","),caption = 'Table 52a. Time From Sign-In to Consent in Minutes Among Active Recruits',booktabs=T, linesep = "", col.names=linebreak(c("Site","Recruitment \n Type","Number \nConsent", "Min","Q1","Median","Mean","SD","Q3","Max")),row.names=FALSE, align=c("l", "c", "c","c", "c", "c", "c", "c", "c"),escape=F) %>% kable_styling(latex_options = "scale_down",font_size=12) %>% add_indent(c(1:9)) %>% footnote(general="Excludes those with sign in time later than consent time (data being corrected).",general_title="Note:",footnote_as_chunk = T)

#colnames(table11_Tm_signin_cst.pas)[4:10,] <- c("Min","Q1","Median","Mean","SD","Q3","Max")
knitr::kable(table52_Tm_signin_cst.pas1[,c(1,2,4:11)],digits=2,format.args = list(big.mark = ","),caption = 'Table 52b. Time From Sign-In to Consent in Minutes Among Passive Recruits',col.names=linebreak(c("Site","Recruitment \n Type","Number \nConsent", "Min","Q1","Median","Mean","SD","Q3","Max")), booktabs=T, linesep = "", row.names=FALSE, align=c("l", "c", "c","c", "c", "c", "c", "c", "c"),escape=F) %>% kable_styling(latex_options = "scale_down",font_size=12) %>% add_indent(c(1:9)) %>% footnote(general="Excludes those with sign in time later than consent time (data being corrected).",general_title="Note:",footnote_as_chunk = T)

colnames(table53_Tm_consent_prof.act1)[5:11] <- c("Min","Q1","Median","Mean","SD","Q3","Max")
knitr::kable(table53_Tm_consent_prof.act1[,c(1,2,4:11)],digits=2,caption = 'Table 53a. Time From Consent to User Profile Submitted in Minutes Among Active Recruits', format.args = list(big.mark = ","), col.names=linebreak(c("Site","Recruitment \n Type","Number UP \nto Submit", "Min","Q1","Median","Mean","SD","Q3","Max")), row.names=FALSE, booktabs = T, linesep = "", align=c("l", "c", "c","c", "c", "c", "c", "c", "c"),escape=F) %>% kable_styling(latex_options = "scale_down",font_size=12) %>% add_indent(c(1:9))
 
colnames(table53_Tm_consent_prof.pas)[5:11] <- c("Min","Q1","Median","Mean","SD","Q3","Max")
knitr::kable(table53_Tm_consent_prof.pas[,c(1,2,4:11)],digits=2,caption = 'Table 53b. Time From Consent to User Profile Submitted in Minutes Among Passive Recruits', format.args = list(big.mark = ","), col.names=linebreak(c("Site","Recruitment \n Type","Number UP \nto Submit", "Min","Q1","Median","Mean","SD","Q3","Max")), row.names=FALSE, booktabs = T, linesep = "", align=c("l", "c", "c","c", "c", "c", "c", "c", "c"),escape=F)  %>% kable_styling(latex_options = "scale_down",font_size=14) %>% add_indent(c(1:9))

colnames(table54_Tm_prof_verif.act)[3:9] <- c("Min","Q1","Median","Mean","SD","Q3","Max")
table54_Tm_prof_verif.act <- table54_Tm_prof_verif.act[,c(1,10,2:9)] 

knitr::kable(table54_Tm_prof_verif.act, digits=2,caption = 'Table 54a. Time From User Profile Submitted to Verification Complete in Days Among Active Recruits', row.names=FALSE, col.names=linebreak(c("Site","Recruitment \n Type","Total UP \nSubmitted", "Min","Q1","Median","Mean","SD","Q3","Max")), format.args = list(big.mark = ","), booktabs = T, linesep = "", align=c("l", "c", "c","c", "c", "c", "c", "c", "c"),escape=F)  %>% kable_styling(latex_options = "scale_down",full_width = F,font_size=12) %>% add_indent(c(1:9))  %>%    column_spec(4,width = "1cm") %>%
...
colnames(table54_Tm_prof_verif.pas)[5:11] <- c("Min","Q1","Median","Mean","SD","Q3","Max")
knitr::kable(table54_Tm_prof_verif.pas[,c(1,2,4:11)],digits=2,caption = 'Table 54b. Time From User Profile Submitted to Verification Complete in Days among Passive Recruits',format.args = list(big.mark = ","),col.names=linebreak(c("Site","Recruitment \n Type","Total UP \nSubmitted", "Min","Q1","Median","Mean","SD","Q3","Max")), row.names=FALSE, booktabs = T, linesep = "", align=c("l", "c", "c","c", "c", "c", "c", "c", "c","c"),escape=F) %>% kable_styling(latex_options = "scale_down",font_size=12) %>% add_indent(c(1:9)) 
# %>% 
...
colnames(table55_Tm_enroll_verif.act)[3:10] <- c("Min","Q1","Median","Mean","SD","Q3","Max","Recruitment Type")
knitr::kable(table55_Tm_enroll_verif.act[,c(1,10,2:9)],digits=2,format.args = list(big.mark = ","),caption = 'Table 45a. Total Enrollment Time in Days Among Active Recruits', col.names=linebreak(c("Site","Recruitment \n Type","Total Verification \nComplete", "Min","Q1","Median","Mean","SD","Q3","Max")), row.names=FALSE, booktabs = T, linesep = "", align=c("l", "c", "c","c", "c", "c", "c", "c", "c","c"),escape=F) %>% kable_styling(latex_options = "scale_down",full_width = F,font_size=14) %>% add_indent(c(1:9)) %>% 
...
colnames(table55_Tm_enroll_verif.pas)[4:10] <- c("Min","Q1","Median","Mean","SD","Q3","Max")
knitr::kable(table55_Tm_enroll_verif.pas[,c(1,12,2,4:10)],digits=2,format.args = list(big.mark = ","),caption = 'Table 55b. Total Enrollment Time From Sign-In to Verification in Days Among Passive Recruits', col.names=linebreak(c("Site","Recruitment \n Type","Total Verification \nComplete", "Min","Q1","Median","Mean","SD","Q3","Max")), row.names=FALSE, booktabs = T, linesep = "", align=c("l", "c", "c","c","c", "c", "c", "c", "c", "c"),escape=F) %>% kable_styling(latex_options = "scale_down",font_size=12) %>% add_indent(c(1:9)) 

table56_age %>% kable_styling(latex_options = "scale_down") %>% footnote(general = 'Based on de-identified data sent by sites',general_title = "Note:",footnote_as_chunk = T)

table57_race %>% footnote(general = 'Based on de-identified data sent by sites',general_title = "Note:",footnote_as_chunk = T)

table58%>% kable_styling(font_size=8) %>% footnote(general = 'Based on de-identified data sent by sites',general_title = "Note:",footnote_as_chunk = T)

table59_ageall  %>% kable_styling(latex_options = "scale_down")%>% footnote(general = 'Based on de-identified data sent by sites',general_title = "Note:",footnote_as_chunk = T)

table60_raceall%>% footnote(general = 'Based on de-identified data sent by sites',general_title = "Note:",footnote_as_chunk = T)

table61_sexall %>% kable_styling(latex_options = "scale_down")%>% footnote(general = 'Based on de-identified data sent by sites',general_title = "Note:",footnote_as_chunk = T)
