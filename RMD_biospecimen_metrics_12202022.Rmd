...
clinic.survey <- recr.biospe %>% filter(d_650516960 == 664882224)
 clinic.survey <- clinic.survey %>% 
   mutate(d_173836415_d_266600170_d_139245758_clc = as.POSIXct(ymd_hms(d_173836415_d_266600170_d_139245758)),
          d_173836415_d_266600170_d_982213346_clc = as.POSIXct(ymd_hms(d_173836415_d_266600170_d_982213346)),
          d_173836415_d_266600170_d_13924575_clc = as.POSIXct(ymd_hms(d_173836415_d_266600170_d_139245758)),
          d_173836415_d_266600170_d_847159717_urine =as.POSIXct(ymd_hms(d_173836415_d_266600170_d_847159717)), #urine
          d_173836415_d_266600170_d_139245758_urine = as.POSIXct(ymd_hms(d_173836415_d_266600170_d_139245758)),
          d_173836415_d_266600170_d_224596428_urine = as.POSIXct(ymd_hms(d_173836415_d_266600170_d_224596428)),
 ...
 clinic.survey <- clinic.survey %>% mutate(clc.coltm  = do.call(pmin, c(select(.,ends_with('clc')),na.rm=TRUE)),
                                           clc.colurinetm = do.call(pmin,c(select(.,ends_with('urine')),na.rm=TRUE)))
...
clc_bld_vt.site<-ggplot(clc.blood.wk_site, aes(x=as_date(ymd(visit.wkdate)), y=n,fill=Site, color=Site)) +
  ...
  scale_x_date(limit=c(as.Date("2022-11-29"),as.Date("2023-01-23")),name="Date",date_labels = "%y-%m-%d", date_breaks = "1 week")  +
  scale_fill_brewer(palette="Dark2") + 
...
knitr::kable(discard.site[,c(5,1:4)], caption='Table 17. Number of Blood Tubes Discarded', row.names=FALSE, col.names=c("Site", "Number of Heparin Tubes Discarded", "Number of ACD Tubes Discarded", "Number of Urine Tubes Discarded", "Total Tubes Discarded"), align=c("l","c","c","c","c","c","c"), booktabs = TRUE) %>% kable_styling(latex_options = "scale_down")
...
write.csv(biospe1,"~/Documents/Connect_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/Connect_prod_Biospe_Formats_02132023.csv",row.names = F,na="")
...
colnames(recrbio1)[which(colnames(recrbio1)==select0)] <-  c(recrvar_CID_dd$Variable.Name[which(recrvar_CID_dd$category==0)][1:26],"Connect_ID","token")
n<- length(select0)
...
write.csv(recrbio1,"~/Documents/Connect_projects/Biospecimen_Feb2022/Jing_projects/biospecQC_03082022/data/Connect_prod_recr_veriBiospe_Formats_02132023.csv",row.names = F,na="")
```
