...
veri_resp <- data[which(data$d_512820379 != 486306141 | data$d_821247024 != 922622075 ),var.list] %>%
  mutate(recru_time = ymd_hms(d_471593703),
         ...
         recrstart.date = as_date(min(ymd_hms(d_471593703),na.rm=TRUE)),
...
veri_act <- filter(veri_resp,d_821247024==197316935) %>% 
  mutate(law.time = ymd_hms(d_264644252),
         ...
         bldcol.time = case_when(d_173836415_d_266600170_d_592099155==534621077 ~ ymd_hms(d_173836415_d_266600170_d_561681068),
                                 d_173836415_d_266600170_d_592099155==664882224 ~ ymd_hms(d_173836415_d_266600170_d_982213346)),
         urine.time = case_when(d_173836415_d_266600170_d_592099155==534621077 ~ ymd_hms(d_173836415_d_266600170_d_847159717),
                                d_173836415_d_266600170_d_592099155==664882224 ~ ymd_hms(d_173836415_d_266600170_d_139245758)),
...
veri_act <- veri_act%>% 
  mutate(module.time  = as_datetime(ifelse(d_100767870 ==353358909,module.tm,ifelse( d_100767870 ==104430631,NA,NA))),
         biocol.time=case_when(d_173836415_d_266600170_d_592099155==534621077 & biospeDonation =="Completed All 3 Sample Donations" ~ do.call(pmax,subset(.,select=c(bldcol.time,urine.time,mw.time),na.rm=TRUE)),
                               d_173836415_d_266600170_d_592099155==664882224 & d_878865966 == 353358909 & d_167958071 == 353358909 ~ do.call(pmax,subset(.,select=c(bldcol.time,urine.time),na.rm=TRUE))))
...
veri_act <- veri_act%>% mutate(allacts = ifelse(module.week >0 & biocol.week>0 & eligible.week >0,353358909,104430631 ))
veri_act$allacts.time <- apply(veri_act[,c("module.time","biocol.time","elgible.time")],1,max)
...
biocol.wk.total <- filter(veri_act,!is.na(biocol.week)) %>% arrange(biocol.week,verified.week) %>% 
  group_by(biocol.week)%>%
  dplyr::summarize(total_biocol=n(),
                   biocol.time.max=max(biocol.time,rm.na=T))

allacts.wk.total <- filter(veri_act,!is.na(allacts.week)) %>% arrange(allacts.week,verified.week) %>% 
...
veri_wk_eligble <- veri_wk_eligble %>% mutate_at(vars(matches("total")),~replace_na(., 0)) %>%  arrange(verified.week)%>% 
...
recr_veri_wk <- recr_veri_wk %>% select(.,-c("verified_time.max.y","total_verifieds.y"))
...
recr_veri_wk <- recr_veri_wk %>% mutate_at(vars(matches("total")),~replace_na(., 0))
...
var.select <- c(  "active_recruits.cum", "passive_verifieds.cum", "total_verified.cum", "cum.total.response.rate", 
                  "active_recruits", "total_verifieds.x", "total_biocol.cum", "biocol_Invites_ratio", "biocol_Verifieds_ratio", "total_biocol", 
                  "total_module.cum", "module_Invites_ratio", "module_Verifieds_ratio", "total_module", "total_eligible.cum", "eligible_Invites_ratio",
...
labels <- c("Cumulative Invitations", "Cumulative Passive Recruits", "Cumulative Verified Participants", 
...
biocol.wk.total.site <- filter(veri_act,!is.na(biocol.week)) %>% arrange(site,biocol.week,verified.week) %>% 
  group_by(site,biocol.week)%>%
  dplyr::summarize(total_biocol=n(),
                   biocol.time.max=max(biocol.time,rm.na=T))

allacts.wk.total.site <- filter(veri_act,!is.na(allacts.week)) %>% arrange(site,allacts.week,verified.week) %>% 
...
recr_veri_wk.site <- recr_veri_wk.site %>% mutate_at(vars(matches("total")),~replace_na(., 0))
...
for(i in 1:length(site)){
  ...
  subs.recr_veri_wk <- subs.recrui_verified %>% arrange(recruit.week)%>% 
    mutate(total_biocol.cum=cumsum(total_biocol),
  ...
  var.select <- c(  "active_recruits.cum", "passive_verifieds.cum", "cum.total_verifieds", "cum.total.response.rate", 
                    "active_recruits", "total_verifieds", "total_biocol.cum", "biocol.invite.ratio", "biocol.verified.ratio", "total_biocol", 
